...
Q&A also supports composite models, if the model contains at least one of the following:

- One import data source
- One DQ data source that supports APPROXIMATEDISTINCOUNT (refer to the previous DirectQuery support section for specific sources)

If the model contains a mix of data sources, we only index import columns or columns for data sources that suport APPROXIMATEDISTINCTCOUNT. This means you can only ask questions about the instance values for these types of columns. You still can ask questions about the columns themselves. 
