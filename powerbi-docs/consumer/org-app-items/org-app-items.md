...
- If a user is removed from an org app, that user loses their org app-based access to included report items and semantic model items associated with that org app. If a user has another form of access to included items or semantic models that form of access is unaffected. (Automatic revocation of included real-time dashboard and notebook items is coming soon.)
...
> **Access revocation**: Org apps propagate access to Fabric items, like real-time dashboard and notebook items included in the app, but do not yet revoke access automatically when a user loses access to an org app or those items are removed from the org app. Manage access removal to these items from each item's permissions management page.
