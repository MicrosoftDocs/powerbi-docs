...
The preview for org apps is default off for tenants. To enable the preview, you must be a Microsoft Fabric administrator. From **Settings > Admin portal > Tenant settings** a Microsoft Fabric administrator needs to enable the switch titled **Users can discover and create org apps (preview)**. Administrators can use security group inclusion / exclusion settings for controlling who can or can't create org apps.
...
1. Create or open the workspace where you want to create org apps 
...
Users in the workspace with an **admin, member, or contributor (with share permissions)*** role can create and manage org app items. Though contributors might not have full permissions for managing access on an org app, more details in the *org app access management* section. Workspace viewers can't create org app items. 

 **See more on contributor permissions in **Contributors and access management / sharing***
...
- Multiple org app items can be created per workspace.
  - With workspace apps, only one app could be created per workspace. If you had the need for different apps within a workspace, you could create audiences that would show or hide content in your app based on permissions. Currently, with org app items, you can create a unique app per need.
...
If you have workspace app configured and published, your org app items don't affect how your workspace app works. You can continue to use workspace apps alongside org app items.
...
Reminder, the users who are given access to an org app item are automatically granted access to... 
