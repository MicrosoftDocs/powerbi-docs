...
We added Android support for remote configuration of the Power BI mobile app's report server access settings. IT admins can now use their organization's MDM tool to remotely configure those settings on both iOS and Android devices. Remote configuration on iOS devices was introduced in [May 2018](#ios-configure-power-bi-ios-mobile-app-access-to-a-report-server-remotely). See [Configure Power BI mobile app access to a report server remotely](../../report-server/configure-powerbi-mobile-apps-remote) for details.
