...
| OneLake | Discover semantic models and other reusable data artifacts. Formerly called Data hub. | Find data, view metadata, request access. | Enterprise discovery surface for governed data you can build reports on. Discoverable items include Fabric real-time artifacts where applicable, and shortcuts and mirrored item types might appear with governed metadata. Improves reuse and reduces duplication. Learn more about [OneLake](/fabric/onelake/onelake-overview).|
| OneLake catalog | Fabric-wide discoverability & governance view for data items. | Search/filter, request access, reuse data. | Broader governed asset catalog (beyond Power BI) for cross-domain analytics. Displays parent–child item hierarchies (for example, Lakehouse with SQL Analytics Endpoint, Eventhouse with KQL databases). AI Auto-Summary for semantic models is available in preview and appears in the catalog when Copilot capacity is enabled. Summaries are generated automatically to help users assess semantic models, and availability is tenant-controlled. Learn more about the [OneLake catalog](/fabric/governance/onelake-catalog-overview). |
| Create | Start new content (report, semantic model, quick create, upload). | Quick create, import files, start metric set. | Launch point for building; options vary by license, role, and enabled Fabric features. The legacy Excel and CSV import experience is being retired; use **Get data** with the current Excel or CSV connectors instead. For details, see [Get data from Excel workbook files](../connect-data/service-excel-workbook-files.md). |
...
| Real-Time\* | Real-time items (event streams, KQL DB) – Fabric feature dependent. | Explore/create real-time artifacts. | Entry point for streaming and low-latency analytical scenarios when Fabric real-time features are enabled. Eventhouse and KQL database items are visible from Real-Time surfaces and within the OneLake catalog. Learn more about [Real-Time Intelligence](/fabric/real-time-intelligence/overview).|
