...
Just like other Power BI Desktop tables, calculated tables can have relationships with other tables. Calculated table columns have data types, formatting, and can belong to a data category. You can name your columns whatever you want and add them to report visualizations just like other fields. Calculated tables are recalculated if any of the tables from which they pull data are refreshed or updated. If the table uses data from DirectQuery, calculated tables aren't refreshed by default. With DirectQuery, the table only reflects the changes after the semantic model is refreshed.
...
1. In the Table View or Model View of Power BI Desktop, select **New table** in the **Calculations** group. It's a bit easier to do in **Table tools** in the Table View, because then you can immediately see your new calculated table.
