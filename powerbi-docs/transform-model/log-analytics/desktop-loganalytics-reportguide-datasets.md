---
title: Use the Log Analytics for Power BI Datasets Template App (preview)
...
---
# Use the Log Analytics for Power BI Datasets Template App (preview)
...
![Diagram showing the major pages of the app and some important available drillthroughs.](media/desktop-loganalytics-reportguide-datasets/template-app-as-engine-flow.png)
...
|Left  | Right  |
|---------|---------|
|**Top N users by operation** - Shows the top 5 users who performed the most operations against the dataset.  **Top N users by query execution** - Shows the top 5 users who ran the most queries against the dataset. |**Queries vs CPU time by users** - Compares query count vs Avg CPU time for each unique query text. |
