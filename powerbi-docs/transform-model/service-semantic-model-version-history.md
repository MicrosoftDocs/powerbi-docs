...
* User information might not appear for versions created by background operations.
* Version history is deleted when a workspace moves between capacities with different BYOK encryption keys or when a capacity’s BYOK encryption key changes.
...
A semantic model may fail to convert to the large semantic model storage format if it exceeds [memory limitations](../enterprise/service-premium-large-models.md) or if the format isn’t supported in the region. The large semantic model storage format is only available in Azure regions that support Azure Premium Files Storage (see the [region availability table](/fabric/enterprise/powerbi/service-premium-large-models)). To resolve memory-related issues, host the model on a Premium capacity SKU that supports the required memory size, or reduce the model size
