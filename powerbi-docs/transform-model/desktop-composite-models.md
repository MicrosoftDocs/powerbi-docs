...
Power BI semantic models can include tables from one or more data sources by using any of the supported [table storage modes](desktop-storage-mode.md). When tables use different storage modes, the model is a composite semantic model. For DirectQuery table storage mode, the model is composite when the DirectQuery tables use different data sources.

For example, if you connect to another Power BI semantic model by using DirectQuery (which adds tables in DirectQuery storage mode) and also have local tables in Import mode, your model becomes a composite model because it contains tables with different storage modes.

> [!NOTE]
> Import tables from one or more data sources aren't composite models until you mix them with non-import tables. The same rule applies to semantic models with Direct Lake tables from one or more data sources.

> [!NOTE]
> For composite models, Direct Lake table storage mode is assumed to be Direct Lake on OneLake. Direct Lake on SQL table storage mode is single source only and can't be added to any composite model. For more information about the differences of Direct Lake table storage mode, see [aka.ms/DirectLake](https://aka.ms/DirectLake).

## Types of composite models

Different types of composite models exist depending on the combination of table storage modes in the semantic model. Each type has its own considerations for functionality and tooling.
...
In Power BI Desktop, you can create semantic models with import or DirectQuery tables locally. You can then add more tables from the **Get data** ribbon button in the other storage mode to create a composite model. 

> [!NOTE]
> If import and DirectQuery tables are both in a semantic model and come from the same data source, dual storage mode is available. Dual mode used instead of DirectQuery can avoid limited relationships with import tables. For more information, see [dual storage mode](/power-bi/transform-model/desktop-storage-mode). 


Adding DirectQuery tables from another Power BI semantic model has a couple different create paths. 

1. In a blank Power BI file, first **connect** to the Power BI semantic model. Once live connected, you have the option to **Make changes to this model**. Selecting **Make changes to this model** from the ribbon or footer converts the live connection to a DirectQuery connection. The DirectQuery connection creates a new local semantic model with the tables in DirectQuery storage mode. You can add new tables in either import or DirectQuery storage mode, as well as give you the option to override some column properties on the source semantic model.

1. In a semantic model with import or DirectQuery tables already, **connect** to a Power BI semantic model and the tables you choose are added as DirectQuery.

Semantic models created with Direct Lake tables are live edited in Power BI Desktop. You can add more Direct Lake tables. To add import tables, open the semantic model in [Power BI web modeling](/power-bi/transform-model/service-edit-data-models). To add DirectQuery tables, use [XMLA](/fabric/enterprise/powerbi/service-premium-connect-tools).

You can live edit a Direct Lake and import semantic model in Desktop, but you can't add more tables. You can only add tables from [Power BI web modeling](/power-bi/transform-model/service-edit-data-models) for Direct Lake and import composite models.

## Create composite models in web modeling

In [Power BI web modeling](/power-bi/transform-model/service-edit-data-models), you can create semantic models with import or Direct Lake tables. You can't add DirectQuery tables. You can add more tables in the other storage mode to create a composite model. 
...
A semantic model combining tables from more than one DirectQuery source, or combining DirectQuery, Direct Lake, and import tables, is a composite semantic model.
...
Each table in a composite model has a storage mode that indicates whether the table is based on DirectQuery or import. You can view and modify the storage mode in the **Properties** pane. To view the storage mode:

1. In **Model** view, select the table.
1. In the **Properties** pane, expand the **Advanced** section, then expand the **Storage mode** list.

You can also view the storage mode on the tooltip for each table when you hover over it in the **Data** pane.
...
Calculated tables are always imported, and their data refreshes when you refresh the tables. If a calculated table refers to a DirectQuery table, visuals that refer to the DirectQuery table always show the latest values in the underlying source. Alternatively, visuals that refer to the calculated table show the values at the time when the calculated table was last refreshed.

> [!IMPORTANT]
> Calculated tables aren't supported in the Power BI service by using this feature unless you meet specific requirements. For more information, see the [Working with a composite model based on a semantic model](#working-with-a-composite-model-based-on-a-semantic-model) section in this article. 

## Security implications

Composite models have some security implications. A query sent to one data source can include data values that are retrieved from another source. In the earlier example, the visual that shows **(Sales Amount)** by **Product Manager** sends a SQL query to the Sales relational database. That SQL query might contain the names of Product Managers and their associated Products.
...
* Any administrator of the database who can view traces or audit logs can view this information, even without permissions to the data in its original source. In this example, the administrator needs permissions to the Excel file.

* The encryption settings for each source. You want to avoid retrieving information from one source by an encrypted connection and then inadvertently including it in a query sent to another source by an unencrypted connection.

To confirm that you considered any security implications, Power BI Desktop displays a warning message when you create a composite model.  
...
For similar reasons, be careful when you open a Power BI Desktop file sent from an untrusted source. If the file contains composite models, information that someone retrieves from one source, by using the credentials of the user who opens the file, is sent to another data source as part of the query. The malicious author of the Power BI Desktop file could view the information. When you initially open a Power BI Desktop file that contains multiple sources, Power BI Desktop displays a warning. The warning is similar to the one displayed when you open a file that contains native SQL queries.  

## Performance implications  

When you use DirectQuery, always consider performance. Ensure that the back-end source has enough resources to provide a good experience for users. A good experience means that the visuals refresh in five seconds or less. For more performance advice, see [DirectQuery in Power BI](../connect-data/desktop-directquery-about.md).

Using composite models adds other performance considerations. A single visual can send queries to multiple sources. Often, one query passes its results to a second source. This situation can result in the following forms of execution:

* **A source query that includes a large number of literal values**: For example, a visual that requests total **Sales Amount** for a set of selected **Product Managers** would first need to find which **Products** those product managers manage. This sequence must happen before the visual sends a SQL query that includes all of the product IDs in a `WHERE` clause.
...
* **Multiple source queries, one per group by value**: When the aggregation uses **DistinctCount** and is grouped by a column from another source, and if the external source doesn't support efficient passing of many literal values that define the grouping, you need to send one SQL query per group by value.

   A visual that requests a distinct count of **CustomerAccountNumber** from the SQL Server table by **Product Managers** imported from the spreadsheet needs to pass in the details from the **Product Managers** table in the query sent to SQL Server. Over other sources, Redshift, for example, this action is unfeasible. Instead, there would be one SQL query sent per **Sales Manager**, up to some practical limit, at which point the query would fail.

Each of these cases has its own implications on performance, and the exact details vary for each data source. Although the cardinality of the columns used in the relationship that joins the two sources remains low (a few thousand), performance shouldn't be affected. As this cardinality grows, pay more attention to the impact on the resulting performance.

Additionally, the use of many-to-many relationships means that separate queries must be sent to the underlying source for each total or subtotal level, rather than aggregating the detailed values locally. A simple table visual with totals sends two source queries, rather than one.
...
If you add another DirectQuery connection to another source, such as a DirectQuery connection to a SQL Server database called **Inventory**, the items from that source are added as another source group:
...
> [!NOTE]
> Importing data from another source doesn't add another source group, because all items from all imported sources are in one source group. Direct Lake and import tables are also considered the same source group.

### Source groups and relationships

A composite model has two types of relationships:

* **Intra source group relationships.** These relationships connect items within a source group. These relationships are always regular relationships unless they're many-to-many, in which case they're limited.
...
Any item in a source group that's a DirectQuery source group is **remote**, unless you define the item locally as part of an extension or enrichment to the DirectQuery source and it's not part of the remote source, such as a measure or a calculated table. A calculated table based on a table from the DirectQuery source group belongs to the "Import" source group and is **local**.
Any item in the "Import" source group is **local**.
For example, if you define the following measure in a composite model that uses a DirectQuery connection to the Inventory source, the measure is local:
...
[Calculation groups](/analysis-services/tabular-models/calculation-groups) help you reduce the number of redundant measures and group common measure expressions together. Typical use cases are time-intelligence calculations where you want to switch from actuals to month-to-date, quarter-to-date, or year-to-date calculations.
When working with composite models, it's important to be aware of the interaction between calculation groups and whether a measure only refers to items from a single remote source group. If a measure only refers to items from a single remote source group and the remote model defines a calculation group that impacts the measure, the calculation group is applied, even if you define the measure in the remote model or in the local model.
However, if a measure doesn't exclusively refer to items from a single remote source group but refers to items from a remote source group on which a remote calculation group is applied, the results of the measure might still be impacted by the remote calculation group. Consider the following example:
...
In this scenario, the **Internet Sales measure** isn't impacted by the calculation group defined in the remote model because they're not part of the same model. However, the calculation group can change the result of the **Reseller Sales** measure, because they're in the same model. This fact means that the results returned by the **Total Sales** measure must be evaluated carefully. Imagine you use the calculation group in the remote model to return year-to-date results. The result returned by **Reseller Sales** is now a year-to-date value, while the result returned by **Internet Sales** is still an actual. The result of **Total Sales** is now likely unexpected, as it adds an actual to a year-to-date result.
...
To create and use composite models on Power BI semantic models, your tenant needs to have the following switches enabled:

- [Allow XMLA Endpoints and Analyze in Excel with on-premises semantic models](/fabric/admin/service-admin-portal-integration#allow-xmla-endpoints-and-analyze-in-excel-with-on-premises-datasets). If you disable this switch, you can't make a DirectQuery connection to a Power BI semantic model.
- [Users can work with Power BI semantic models in Excel using a live connection](/fabric/admin/service-admin-portal-export-sharing#users-can-work-with-power-bi-datasets-in-excel-using-a-live-connection). If you disable this switch, users can't make live connections to Power BI semantic models so the **Make changes to this model** button isn't available.
...
Tenant administrators can enable or disable DirectQuery connections to Power BI semantic models in the admin portal. While this setting is enabled by default, disabling it stops users from publishing new composite models on Power BI semantic models to the service.
...
Existing reports that use a composite model on a Power BI semantic model continue to work. Users can still create the composite model in Power BI Desktop but can't publish it to the service. Instead, when you create a DirectQuery connection to the Power BI semantic model by selecting **Make changes to this model**, you see the following warning message:
...
This way you can still explore the semantic model in your local Power BI Desktop environment and create the composite model. However, you can't publish the report to the service. When you publish the report and model, you see the following error message and publication is blocked:
...
Live connections to Power BI semantic models aren't influenced by the switch, nor are live or DirectQuery connections to Analysis Services. These connections continue to work regardless of the switch setting. Also, any published reports that use a composite model on a Power BI semantic model continue to work even if the switch is turned off after they were published.


### Building a composite model on a semantic model or model

To build a composite model on a Power BI semantic model or Analysis Services model, your report needs a local model. You can start from a live connection and add or upgrade to a local model, or start with a DirectQuery connection or imported data, which automatically creates a local model in your report.

To see which connections are used in your model, check the status bar in the bottom right corner of Power BI Desktop. If you're only connected to an Analysis Services source, you see a message like the following image:

![Screenshot showing Analysis Services only connection.](media/desktop-composite-models/directquery-datasets-01.png)

If you're connected to a Power BI semantic model, you see a message that tells you which Power BI semantic model you're connected to:

![Screenshot showing Power BI semantic model connection.](media/desktop-composite-models/directquery-datasets-01b.png)

If you want to customize the metadata of fields in your live connected semantic model, select **Make changes to this model** in the status bar. Alternatively, you can select the **Make changes to this model** button in the ribbon, as shown in the following image. In **Report View** the **Make changes to this model** button is in the **Modeling** tab. In **Model View**, the button is in the **Home** tab.

![Screenshot showing Make changes to this model button.](media/desktop-composite-models/directquery-datasets-02.png)

When you select the button, a dialog appears that confirms the addition of a local model. Select **Add a local model** to enable creating new columns or modifying the metadata for fields from Power BI semantic models or Analysis Services. The following image shows the dialog. 
...
Semantic models and the semantic models on which they're based form a *chain*. This process, called *chaining*, lets you publish a report and semantic model based on other Power BI semantic models.

For example, imagine your colleague publishes a Power BI semantic model called *Sales and Budget* based on an Analysis Services model called *Sales*, and combines it with an Excel sheet called *Budget*. Then you create and publish a composite semantic model and report, called *Sales and Budget Europe*, using the *Sales and Budget* Power BI semantic model with your own modifications. This semantic model is third in the chain. 

1. The first chain is the *Sales* Analysis Services model.
1. The second chain is the *Sales and Budget* Power BI composite semantic model.
1. The third chain is your *Sales and Budget Europe* Power BI composite semantic model.
...
Users accessing reports by using a composite model need proper permissions to [all semantic models and models in the chain](#chaining).

The owner of the composite model needs **Build** permission on the semantic models used as sources so that other users can access those models on behalf of the owner. As a result, creating the composite model connection in Power BI Desktop or authoring the report in Power BI requires **Build** permissions on the semantic models used as sources.

Users who view reports by using the composite model generally need **Read** permissions on the composite model itself and the semantic models used as sources. **Build** permissions might be required if the reports are in a Pro workspace. [These tenant switches](#managing-composite-models-on-power-bi-semantic-models) should be enabled for the user.

The following example illustrates the required permissions:

* **Composite Model A** (owned by **Owner A**)
    - Data source A1: **Semantic Model B**.<br/>**Owner A** must have **Build** permission on **Semantic Model B** for users to view the report that uses **Composite Model A**.

* **Composite Model C** (owned by **Owner C**)
    - Data source C1: **Semantic Model D**<br/>**Owner C** must have **Build** permission on **Semantic Model D** for users to view the report that uses **Composite Model C**.
...
A user viewing reports that use **Composite Model A** must have **Read** permissions to both **Composite Model A** and **Semantic Model B**, while a user viewing reports that use **Composite Model C** must have **Read** permissions on **Composite Model C**, **Semantic Model D**, **Composite Model A**, and **Semantic Model B**.
...
When you use the **Composite models on Power BI semantic models and Analysis Services models** feature, you see a security warning dialog, shown in the following image.

![Screenshot showing Security warning.](media/desktop-composite-models/directquery-datasets-05.png)

Data might be pushed from one data source to another data source. This security warning applies to combining DirectQuery and import sources in a data model. For more information about this behavior, see [using composite models in Power BI Desktop](../transform-model/desktop-composite-models.md).

### Supported scenarios

You can build composite models by using data from Power BI semantic models or Analysis Services models to service the following scenarios: 

- Connect to data from various sources: Import (such as files), Power BI semantic models, Analysis Services models
- Create relationships between different data sources
- Write measures that use fields from different data sources
- Create new columns for tables from Power BI semantic models or Analysis Services models
- Create visuals that use columns from different data sources
- Remove a table from your model by using the field list, to keep models as concise and lean as possible (if you connect to a perspective, you can't remove tables from the model) 
- Specify which tables to load, rather than having to load all tables when you only want a specific subset of tables. See Loading a subset of tables later in this document. 
- Specify whether to add any tables that you later add to the semantic model after you make the connection in your model. 
...
* To build reports in the Power BI service on a composite model based on another semantic model, you must set all credentials.  
...
* All connections to remote Power BI semantic models use single sign-on. Authenticating with a service principal isn't currently supported. 
* RLS rules apply on the source on which they're defined, but don't apply to any other semantic models in the model. RLS defined in the report doesn't apply to remote sources, and RLS set on remote sources don't apply to other data sources. Also, you can't define RLS on a table loaded from a remote source, and RLS defined on local tables don't filter any tables loaded from a remote source. 
* KPIs, row level security, and translations aren't imported from the source. 
* You might see some unexpected behavior when using a date hierarchy. To resolve this issue, use a date column instead. After adding a date hierarchy to a visual, you can switch to a date column by selecting the down arrow in the field name, then selecting the name of that field instead of using Date Hierarchy: 
...
* You can set a discourage chaining flag on a model to prevent a chain from being created or extended. For more information, see [Manage DirectQuery connections to a published semantic model](../connect-data/desktop-discourage-directquery-connections-to-dataset.md).
* Power Query doesn't show the connection to a Power BI semantic model or Analysis Services model.
...
* Publishing a report to web by using the publish to web feature isn't supported.
...
* If you rename a workspace after setting up the DirectQuery connection, you need to update the data source in Power BI Desktop for the report to continue working.
...
* Taking over a semantic model that's using the **DirectQuery to other semantic models** feature isn't currently supported.
* As with any DirectQuery data source, hierarchies defined in an Analysis Services model or Power BI semantic model don't show when connecting to the model or semantic model in DirectQuery mode by using Excel.

When working with DirectQuery for Power BI semantic models and Analysis Services, consider the following guidance:
...
In the diagram, Ash works with Contoso and accesses data provided by Fabrikam. With Power BI Desktop, Ash creates a DirectQuery connection to an Analysis Services model that Fabrikam hosts.

To authenticate, Ash uses a B2B guest user identity (step 1 in the diagram).

If Ash publishes the report to Contoso’s Power BI service (step 2), the semantic model published in the Contoso tenant can't successfully authenticate against Fabrikam’s Analysis Services model (step 3). As a result, the report doesn't work.

In this scenario, since Fabrikam hosts the Analysis Services model, you must also publish the report in Fabrikam's tenant. After successful publication in Fabrikam’s tenant (step 4), the semantic model can successfully access the Analysis Services model (step 5) and the report works properly.
...
Object-level security (OLS) enables model authors to hide objects that make up the model schema (that is, tables, columns, metadata, and so on) from model consumers (for example, a report builder or a composite model author). In configuring OLS for an object, the model author creates a role, and then removes access to the object for users who are assigned to that role. From the standpoint of those users, the hidden object simply doesn't exist.

OLS is defined for and applied on the source model. You can't define it for a composite model built on the source model.

When you build a composite model on top of an OLS-protected Power BI semantic model or Analysis Services model via DirectQuery connection, you copy the model schema from the source model into the composite model. What you copy depends on what you're permitted to see in the source model according to the OLS rules that apply there. You don't copy the data to the composite model – you always retrieve it via DirectQuery from the source model when needed. In other words, data retrieval always gets back to the source model, where OLS rules apply.

Since the composite model isn't secured by OLS rules, the objects that consumers of the composite model see are those that you could see in the source model rather than what they themselves might have access to. This situation might result in the following outcomes:
...
1. Admin_user publishes an enterprise semantic model using a Power BI semantic model or an Analysis Services model that has a Customer table and a Territory table. Admin_user publishes the semantic model to the Power BI service and sets OLS rules that have the following effect:
...
* You can't define OLS rules on the composite model itself.
* A consumer of a composite model never sees actual data they aren't supposed to see, because relevant OLS rules on the source model block them when DirectQuery tries to retrieve the data using their credentials.
...
In the previous example, we added ' (marketing)' as a suffix to any table or measure name that conflicts with another source in the composite model. You can: 
- Enter a text to add to the name of conflicting tables or measures.
...
- Choose to apply the deduplication rule only when a name conflict occurs or apply it all the time. The default is to apply the rule only when duplication occurs. In our example, any table or measure from the marketing source that doesn't have a duplicate in the sales source doesn't get a name change. 

After you make the connections and set up the deduplication rule, your field list shows both 'Customer' and 'Customer (marketing)' according to the deduplication rule set up in our example:
...
If you don't specify a deduplication rule, or the deduplication rules you specify don't resolve the name conflict, the standard deduplication rules are still applied. The standard deduplication rules add a number to the name of the conflicting item. If there's a name conflict on the 'Customer' table, one of the 'Customer' tables is renamed 'Customer 2'.


## XMLA modifications and composite models

When you change a semantic model by using XMLA, update the *ChangedProperties* and *PBI_RemovedChildren* collections for the changed object to include any modified or removed properties. If you don't update these collections, Power BI modeling tools might overwrite your changes the next time the schema synchronizes with the data source.

For more information about semantic model object lineage tags, see [lineage tags for Power BI semantic models](/analysis-services/tom/lineage-tags-for-power-bi-semantic-models).
...
**Mixed-mode connections** - When you use a mixed mode connection that contains online data (such as a Power BI semantic model) and an on-premises semantic model (such as an Excel workbook), you must establish gateway mapping for visuals to properly appear.
...
If you use row-level security in a composite model with some of the tables in DirectQuery mode, you must refresh the model to apply new updates from the DirectQuery tables. For example, if a Users table in DirectQuery mode has new user records at the source, the new records are included only after the next model refresh. Power BI Service caches the Users query to improve performance and doesn’t reload the data from the source until the next manual or scheduled refresh.
