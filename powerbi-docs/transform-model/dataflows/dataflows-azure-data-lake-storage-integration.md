...
The **Use default Azure connection** option is visible if the tenant has already configured ADLS Gen 2. You have two options: 

- use the tenant configured ADLS Gen 2 by selecting the box called **Use the default Azure connection**, or     
...
Once selected, select **Save** and you now have successfully connected the workspace to your own ADLS Gen2 account. Power BI automatically configures the storage account with the required permissions, and sets up the Power BI filesystem to which  the data will be written. At this point, every dataflow’s data inside this workspace will write directly to this filesystem, which can be used with other Azure services, creating a single source for all of your organizational or departmental data.
...
We only write to this storage account and do not currently delete data. This means that even upon detach, we don’t delete from the ADLS account, so all of the above files are still stored. 
...
If you are connecting ADLS Gen 2 to Power BI, you can do this at the workspace or tenant level. Make sure you have the right access control. Learn more by visiting [Configuring dataflow storage to use Azure Data Lake Gen 2](https://docs.microsoft.com/power-bi/transform-model/dataflows/dataflows-azure-data-lake-storage-integration).
 
The storage structure adheres to the Common Data Model format. Learn more about the storage structure and CDM by visiting [What is the storage structure for analytical dataflows](https://docs.microsoft.com/power-query/dataflows/what-is-the-cdm-storage-structure-for-analytical-dataflows) and [Common Data Model and Azure Data Lake Storage Gen2](https://docs.microsoft.com/common-data-model/data-lake).
