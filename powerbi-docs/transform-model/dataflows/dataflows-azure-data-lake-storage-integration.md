...
- The user must have Azure Blob Data Owner role, Azure Blob Data Reader role, and a Owner role at the storage account level (scope should be 'this resource' and not inherited).  Note:  Applied role changes may take a few minutes to sync before the steps below can be completed in the Power BI Service. 
