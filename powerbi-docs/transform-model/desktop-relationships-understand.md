...
The first step to achieve an effective and efficient model design is to develop a well-designed model. We recommend you apply [star schema](../guidance/star-schema.md) design principals to produce a model comprising dimension and fact tables. It’s common to set up Power BI to enforce rules that filter dimension tables, allowing model relationships to efficiently propagate those filters to fact tables.
