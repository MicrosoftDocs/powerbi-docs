...
We recommend you apply [star schema](../guidance/star-schema.md) design principals to produce a model comprising dimension and fact tables. It’s common to set up Power BI to enforce rules that filter dimension tables, allowing model relationships to efficiently propagate those filters to fact tables.
...
- [USERELATIONSHIP](/dax/userelationship-function-dax): Forces the use of a specific inactive model relationship. It's useful when your model includes a role-playing dimension table, and you choose to create inactive relationships from this table.
...
The following example shows what priority paths that can be taken between a filter source and target will be given following the above precedence rules:

:::image type="content" source="media/desktop-relationships-understand/relationship-precedence-rules.png" alt-text="A diagram showing multiple filter paths and show their precedence.":::

PREVIOUS INFO BELOW (to be deleted):
The following list presents precedence rules that Power BI uses for ambiguity detection and path resolution:
