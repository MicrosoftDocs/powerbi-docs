...
This article targets Import data modelers working with Power BI Desktop. It is a very important model design topic that is essential to delivering intuitive, accurate, and optimal models.

For a deeper discussion on optimal model design, including table roles and relationships, read the [Understand star schema and the importance for Power BI](guidance/star-schema) article.
...
> [!IMPORTANT]
> It is important to understand that model relationships do not enforce data integrity. Refer to the [Relationship evaluation](#relationship-evalutaion) topic later in this article to learn how model relationships can respond to data integrity issues in your data.

Let's see how relationships can propagate filters with a simple example supported by an animated image.

![Animated example of relationship filter propagation](media/animation-filter-propagation.gif)
...
When the cross filter direction is set to **Both** , an additional property is available to ensure that bi-directional filtering takes place when row-level security (RLS) rules are enforced. For further information about RLS, read the [Row-level security (RLS) with Power BI Desktop](desktop-rls) article.
...
There can only be one active filter propagation path between two model tables. However, it is possible to introduce additional relationship paths, though these relationships must all be configured as _inactive_. Inactive relationships can only be made active during the evaluation of a model calculation. This is achieved by using the [USERELATIONSHIP](/dax/userelationship-function-dax) DAX function.
...
This property should always be enabled when a database foreign key constraint exists between the two tables. When a foreign key constraint does not exist, you can still enable the property providing that you are certain data integrity exists.
...
>
> For further information, read the [Assume referential integrity settings in Power BI Desktop](desktop-assume-referential-integrity) article.
...
Model relationships, from an evaluation perspective, are classified as either _strong_ or _weak_. This is not a configurable relationship property. It is in fact inferred from the cardinality type or the source of the two related model tables. It is important to understand the evaluation type because there may be performance implications or data integrity consequences, as will be described in this topic.
...
![Example of a Composite model consisting of two islands](media/data-island-example.png)
...
![Example of a Composite model consisting of two islands with strong relationships marked](media/data-island-example-strong.png)
...
For Many-to-many relationships, the table expansion takes place from the "many" to the "one" sides by using LEFT OUTER JOIN semantics. When a matching value from the "many" to the "one" side does not exist, then a blank virtual row is added to the "one" side table.
...
The blank virtual rows are effectively "Unknown Members". They represents referential integrity violations where the "many" side value has no corresponding "one" side value. Ideally these blanks do not exist, and this can be achieved by cleansing and repairing the source data.

Let's see how table expansion can works with a simple example supported by an animated image.

![Animated example of table expansion](media/animation-expanded-table.gif)
...
![Example of a Composite model consisting of two islands with weak relationships marked](media/data-island-example-weak.png)
...
- The **RELATED** DAX function cannot be used to retrieve the "one" side columns values
- Enforcing RLS has topology restrictions

> [!NOTE]
> In Power BI Desktop model view, it is not always possible to determine whether a model relationship is strong or weak. A Many-to-many relationship will always be weak. However, a One-to-many relationship may be weak, providing it is a cross-island relationship. To determine whether it is a cross-island relationship, you will need to inspect the table storage modes and data sources to arrive at a correct determination.
...
- [Understand star schema and the importance for Power BI](guidance/star-schema)
