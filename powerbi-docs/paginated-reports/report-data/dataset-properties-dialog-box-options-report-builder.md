---
title: "Dataset Properties Dialog Box, Options (Power BI Report Builder)"
...
ms.reviewer: rpatkar
---
# Dataset Properties Dialog Box, Options (Power BI Report Builder)
...
## Options  
 **Collation**  
 Select a locale that determines the collation sequence to be used for sorting data. **Default** indicates that the Power BI service should attempt to derive the value from the data provider when the report runs. If the value cannot be derived, the default value is derived from the locale setting of the node running the report.  
