...
Read about Report Builder rendering extensions for paginated reports so you're sure your report looks the way you want, including page layout, page breaks, and paper size. 
 ...
 When you view reports in on the service at https://powerbi.com or the preview pane of Report Builder, the report is first rendered by the default renderer. You can then export the report to different formats such as Excel or comma-delimited (CSV) files. The exported report can then be used for further analysis in Excel or as a data source for applications that can import and use CSV files.  
