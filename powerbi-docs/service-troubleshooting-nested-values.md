...
Starting with June Power BI, when the Firewall buffers a nested table/record/list/etc., instead of silently converting such values to text, an error will be produced that says: “We cannot return a value of type Table in this context”.

## Effect on Load/Refresh

While the change is motivated by Firewall buffering, its impact extends to Load/Refresh as well. In order to address the Firewall buffering behavior, we also had to change the behavior of loading nested tables/records/etc. to the Power BI Model, as well as Excel sheets and the Excel Data Model in PQ for Excel. The good news is that this is something we’ve wanted to do for a long time now. Whereas before, nested tables/records/etc. would be loaded as text values (such as “[Table]” or “[Record]”), they will now be treated as errors, which results in a null value in the loaded table and an error count increment in the load results.
...
Are you loading a column that contains non-scalar values (e.g. tables, lists, records, etc.)?
If so, you should be able to eliminate the errors by removing the column.
If you cannot remove the column, you should be able to replicate the old behavior by adding a custom column and using logic like the following:
