...
| Tooltip options for Power BI visuals (Generally Available) | Configure tooltip behavior in the format pane, including showing only fields specified in the build pane and adding custom text. Text mode lets you reference tooltip fields to craft a sentence instead of listing fields. For more information, see [Visual tooltips in Power BI](../visuals/power-bi-visualization-visual-tooltips.md). |        |
