...
1. Select **Continue**.
   ...
   > [!NOTE]
   > If you've connected to Log Analytics from Power BI before, you're asked to choose which credentials to use to for the connection between Power BI and Log Analytics before being taken to the semantic model details page. For help deciding which credentials to choose, see [Choosing which credentials to authenticate with](#choosing-which-credentials-to-authenticate-with).
...
If you get the following dialog, it means that you established a connection to Log Analytics in the past. The credentials that you used at that time might be different from the credentials of your current sign in. Choose whether to continue using the sign-in details you used the last time you connected (*The credentials I used to connect to Power BI last time*), or whether the connection should use your current sign-in credentials from now on (*My current credentials (these may be the same or different)*).
...
If you replace the credentials you used last time with your current sign-in credentials, the data you see in the semantic model you're creating is exactly the same as what you see in Log Analytics. However, since the connection now uses your current authentication credentials, views of the data in semantic models you might have created previously from that Log Analytics query might also change, and this change could affect reports and other downstream items that users created based on those semantic models.

If this is your first time connecting to Log Analytics from Power BI, Power BI automatically uses your current credentials to establish the connection, and you don't see this dialog.
