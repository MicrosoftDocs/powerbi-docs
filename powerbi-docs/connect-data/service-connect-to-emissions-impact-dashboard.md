...
> [!IMPORTANT]
> In January 2024, your data will undergo recalculation due to an update in the methodology that now allows for a more detailed attribution of carbon emissions. For more information about these changes, go to [Calculations update FAQ](#calculations-update-faq).
...
The source of the data for both is the same. Emissions Impact Dashboard for Azure has data at the subscription level and the Cloud for Sustainability API has the same data at the resource level.
