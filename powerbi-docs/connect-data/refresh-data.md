...
Significant use of dashboard tiles or premium caching can increase refresh duration, since either can queue many queries after each refresh. You can either reduce the number of dashboards or [disable automatic cache refresh](/analysis-services/server-properties/general-properties?view=power-bi-premium-current&preserve-view=true) setting to help reduce the number of queries.
...
To cancel a semantic model refresh, you need to be a contributor, member or an admin of the semantic model's workspace. Dataset refresh cancellation only works with semantic models that use [import mode](./../connect-data/service-dataset-modes-understand.md#import-mode) or [composite mode](./../connect-data/service-dataset-modes-understand.md#composite-mode).
