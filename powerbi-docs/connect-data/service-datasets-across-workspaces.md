...
- Building a report on top of a dataset in a different workspace requires a shared workspace at both ends: The report needs to be in a shared workspace and the dataset needs to be in a shared workspace. You can only copy reports in a shared workspace to another shared workspace, and not to My Workspace.
