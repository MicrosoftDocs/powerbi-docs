...
> [!NOTE]
>  In certain situations, performing equality related comparisons (=, \< \>, \>= and \<=) between values that have their data type set to Decimal Number might return unexpected results. This is most apparent when using the [RANKX function](/dax/rankx-function-dax): in some cases the results returned by RANKX are incorrect. The reason this happens is because values with data type Decimal Number are stored as IEEE Standard 754 floating point numbers and as a result have inherent limitations in their precisions. To avoid unexpected results, consider changing the data type to Fixed Decimal Numbers (see below) or do a forced rounding using [ROUND](/dax/round-function-dax).
> 
>
