...
**Date column** - The table must contain a date column of date/time or integer data type. The RangeStart and RangeEnd parameters (which must be date/time data type) filter table data based on the date column. For date columns of integer surrogate keys in the form of `yyyymmdd`, you can create a function that converts the date/time value in the RangeStart and RangeEnd parameters to match the integer surrogate keys of the date column. To learn more, see [Configure incremental refresh - Convert DateTime to integer](incremental-refresh-configure.md#convert-datetime-to-integer).  For non-SQL data sources, the RangeStart/RangeEnd parameters can be inserted into the source query to specify a range file or folder names with dates in them, or pasted into a URL in a Web data source.

**Query folding** - Incremental refresh is designed for data sources that support query folding, which is Power Query's ability to generate a single query expression to retrieve and transform source data, especially if getting the latest data in real time with DirectQuery. If the RangeStart/RangeEnd parameters are not used in the initial source query step, it's important that the query steps up through where the RangeStart/RangeEnd parameters are used can be folded into the source query.  If the RangeStart/RangeEnd parameters can't be folded into the source query will apply the filter locally, which requires retrieving all rows for the table from the data source. 

If the incremental refresh policy includes getting real-time data with DirectQuery, non-folding transformations can't be used.  If using Detect Data Changes with a non-folding query, you must provide a custom query to detect data changes.
