...
To dig down into the report data, or to build your own reports against the dataset, use **Save as** (see [Save the Usage Metrics report as a full-featured Power BI report](#save-the-usage-metrics-report-as-a-full-featured-power-bi-report).
...
A:    Usage metrics currently does not support capturing usage for embedded dashboards and reports, including the [user owns data](powerbi-developer-integrate-report.md) flow, the  [app owns data](developer/embed-sample-for-customers.md) flow and the [publish to web](service-publish-to-web.md) flow. In those cases, we recommend using existing web analytics platforms to track usage for the hosting app or portal.
