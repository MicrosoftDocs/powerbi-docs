...
This setting applies to (a) report rendering and (b) the query cache refresh and tile refresh portions of a refresh operation. A [refresh operation](refresh-data.md) consists of a data refresh (i.e. import of data from the data source) followed by an optional query cache refresh and optional tile refresh. This memory limit also applies to the queries in the query cache refresh and tile refresh, where any query failure during those refreshes will cause the scheduled refresh state to be reported as a failure, even though the data refresh was successful.

#### Query Timeout

Use this setting to maintain better control of long-running queries, which can cause reports to load slowly for users. This setting applies to (a) report rendering and (b) the query cache refresh and tile refresh portions of a refresh operation. A [refresh operation](refresh-data.md) consists of a data refresh (i.e. import of data from the data source) followed by an optional query cache refresh and optional tile refresh. This timeout limit also applies to the queries in the query cache refresh and tile refresh, where any query failure during those refreshes will cause the scheduled refresh state to be reported as a failure, even though the data refresh was successful.
