...
- Paginated Report Visual has a built-in 30,000 data reduction limit. If there is any field used in the visual contains more than 30,000 values, data reduction algorithm will kick in and drop the rows above the limit. As a result, the paginated report may show incorrect result due to missing data from report parameters. To workaround this, try to use filters to filter out unnecessary values; or map the report parameters to fields with fewer values.
- The duration of the paginated report session is capped by the expiration time of user's access token. Currently there's no support to automatically refresh the token. After session expired, there will be a dialog prompt to refresh the visual to start a new render session.
