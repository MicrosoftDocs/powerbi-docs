...
- When you add a Power BI report that contains a paginated report visual to a Power BI app, make sure to include the paginated report (as well as all subreports and drillthrough target reports the report visual depends on) in the app. If your app is not upgraded to support audiences or if the paginated report is in another workspace, your app users must have at least Viewer role access to the paginated report workspace in order to view the paginated report in the visual.
