...
1. You can also format individual columns and headers.
   ...
   1. Finally, configure the other **Apply settings to** options to specify how to use the updated settings for the column data.
      ...
      Let's turn on the **Apply to values** and **Apply to total** options.
...
1. Practice what you've learned by updating another specific column field.
   ...
   1. For the **Values** options, specify **Center** for the **Alignment**.
...
1. Select a contiguous range of cells or hold the **CTRL** key and to choose multiple cells that aren't contiguous.
...
- When you apply column formatting, you can choose only one alignment method per column: **Auto**, **Left**, **Center**, or **Right**. Usually, a column contains all text or all numbers, not a mix of values. In cases where a column contains both numbers and text, the **Auto** option aligns left for text and right for numbers. This behavior supports languages where you read left-to-right.

- If text data in table cells or headers contains new line characters, the characters are ignored by default. If you want Power BI to recognize these formatting characters, enable the **Values** > **Values** > **Text wrap** option for the specific element in the **Format** section of the **Visualizations** pane.
