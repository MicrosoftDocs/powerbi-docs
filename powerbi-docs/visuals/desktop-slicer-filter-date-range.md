...
>[!Note]
>If your report has a Date hierarchy with your data column from the [auto date/time feature](power-bi/transform-model/desktop-auto-date-time), the date column must be used instead of the date hierarchy to use the slicer in relative date mode.
...
- [Auto date/time in Power BI](power-bi/transform-model/desktop-auto-date-time)
