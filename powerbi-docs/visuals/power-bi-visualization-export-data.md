...
- For export from matrix visuals using *Data with current layout*, consider the following:
  ...
  - If the **Switch values to rows** formatting option is set to 'On' in Power BI for a table or matrix visual, the visual format is not preserved when data is exported to Excel
