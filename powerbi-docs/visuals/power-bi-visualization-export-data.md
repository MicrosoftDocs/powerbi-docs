...
:::image type="content" source="media/power-bi-visualization-export-data/power-bi-introduction.png" alt-text="Screenshot of Power BI service showing dropdown for More actions.":::
...
:::image type="content" source="media/power-bi-visualization-export-data/power-bi-contact.png" alt-text="Screenshot of dashboard displaying contact information.":::

## Data is protected when exported out of Power BI

Report owners can classify and label reports by using [sensitivity labels](../enterprise/service-security-data-protection-overview.md) from Microsoft Purview Information Protection. If the sensitivity label has protection settings, Power BI applies these protection settings when exporting report data to Excel, PowerPoint, or PDF files. Only authorized users can open protected files.
...
1. If the tile comes from a report with a sensitivity label, you see this warning. Consider the sensitivity of your content before deciding whether to export or not. To export data with a sensitivity label, open the report page that contains the original visual and export the data from there instead.  
...
1. In the **Save As** dialog box, select a location for the .csv file, and edit the file name, if you want.
1. Select **Save**.
...
- A [hierarchy](../create-reports/desktop-drillthrough.md) made up of **Country/Region** and **City**. With that hierarchy, you can drill down from **Country/Region** to **City**, and back up again.
- A filter for **City**.
- An aggregate for **Discount percent** (count was changed to average).

:::image type="content" source="media/power-bi-visualization-export-data/power-bi-numbered.png" alt-text="Screenshot of chart with numbered labels.":::

Now you're ready to try out two different options for exporting data. Select **More options (...)** in the upper-right corner of the visual, and then choose **Export data**.

:::image type="content" source="media/power-bi-visualization-export-data/power-bi-export-visual.png" alt-text="Screenshot of visual dropdown showing option for selecting Export data.":::
...
:::image type="content" source="media/power-bi-visualization-export-data/power-bi-export-option.png" alt-text="Screenshot of window showing options for Current layout, Summarized data, and Underlying data.":::

### Export summarized data

Select the option for **Summarized data** if you want to export data for what you see in that visual. This type of export shows you only the data (columns and measures) that the visual uses. Since this visual has a hierarchy, your export contains aggregated data for the full hierarchy. So, even though your current view of the chart shows two columns (two Country/Regions), your summarized data display four rows - one for each **City** in the hierarchy.
...
- Excel .xlsx file (up to 150,000 rows max). When you choose this option, the exported file inherits the sensitivity label (if any) of the report.
- .csv file (up to 30,000 rows max)
...
When you select  **Export**, your browser prompts you to save the file. Once saved, open the file in Excel. If you're using the Power BI app in Microsoft Teams, you might not receive the same prompts. Your exported file is saved in your local Downloads folder or in a folder that you specify.
...
- All the data used by the hierarchy is exported, not simply the data used for the current drill level for the visual. For example, you didn't yet drill down to the city level, but your export includes city data as well as country/region data.

- Your exported data is aggregated. You get a total percentage, one row, for each city.

- Since you applied filters to the visualization, the exported data exports as filtered. Notice that the first row displays **Applied filters: City isn't Atlanta, GA**.
...
For some data, **underlying** is disabled. You might need build permissions to see more data than the visual currently displays (underlying data). These permissions protect the data from being inappropriately viewed, reused, or shared with anyone other than the intended audience.
...
- All the data used by the hierarchy is exported, not simply the data used for the current drill level for the visual. For example, you didn't yet drill down to the city level, but your export includes both city and country/region data.

- Since you applied filters to the visual, the exported data exports as filtered. The first row displays **Applied filters: City isn't Atlanta, GA**. In some specific cases, the first row might display a filter that isn't being applied in the data that follows. See the next section for details.
...
1. Under **CURRENT FILE**, select **Report settings**.
   ...
   :::image type="content" source="media/power-bi-visualization-export-data/desktop-report-settings.png" alt-text="Screenshot of the desktop report settings.":::
...
When contacting the report owner or administrator, refer them to these articles:
