...
1. Open the report, and from the top menu bar, select **Subscribe** :::image type="icon" source="./media/end-user-subscribe/power-bi-icon.png" border="false":::.  
1. Select **Create a subscription** and give your subscription a name. By default, the subscription has the same name as your report page or dashboard. Optionally, change the name of the subscription to something more meaningful.
...
1. Use the green slider to turn the subscription on or off. Setting the slider to **Off** doesn't delete the subscription. To delete the subscription, select the trashcan icon.
...
1. Optionally, add a subject and message for recipients. For reports, this message appears above the preview in the body of the email. For dashboards, the message appears within the generated preview, at the top of the image.
...
1. Include additional information in the email. Select one or more of these checkboxes.
    - **Permission to view the report/dashboard in Power BI**: Give recipients permission to open and view the report or dashboard in the Power BI service (app.powerbi.com). This option is only available when you have sufficient permissions in the workspace (Contributor, Member, or Admin role). When selected, recipients who don't already have access receive permission to view the content. For more information about workspace roles and permissions, see [Roles in workspaces](service-roles-new-workspaces.md).
    ...
    > [!CAUTION]
    > Sensitivity labels aren't applied to a subscription's email or preview image. To protect against this, turn off **Preview image**.  
...
> - You can have Power BI send subscription emails to a [mail-enabled security group](end-user-subscribe.md#use-group-email-aliases). Be sure to add the Power BI alias (`no-reply-powerbi@microsoft.com`) to the approved sender list.

## Sensitivity labels

The system applies sensitivity labels to subscription email attachments but not to the subscription's email text or preview images.
...
You don't see the option to **Include my changes** until you make changes to a report that was shared with you. After you make the changes that you need, open an existing subscription or create a new subscription for that report page. Because you made changes to the original report, you now have the header for **My changes** and the option to **Include my changes**.
...
1. Select **More options** > **Include my changes** > **Preview** to display a view-only version of the original subscription that the service sends to subscribers. Use this comparison to decide if you want to overwrite the original state of the subscribed report.
...
> [!NOTE]
> The update applies to the current page only. Changes you made to other pages aren't included. To include changes made to a different page in the report, go to that page and apply an update.
...
You see all the subscriptions you created in the currently selected workspace. For each subscription, Power BI displays the name you gave to the subscription, the name of the content you're subscribing to, and the content type. If you select **Include my changes**, the **State** column includes the date. Search for subscriptions by keyword or filter by any of these fields.
...
- open the report or dashboard by selecting **Edit in report** or **Edit in dashboard**.
    ...
    :::image type="content" source="media/end-user-subscribe/power-bi-edit.png" alt-text="Screenshot showing the Subscriptions with Edit in report outlined in red.":::
...
You see a list of all your subscriptions across all workspaces. Power BI displays the name of the subscription, the name of the content you're subscribing to, the name of the owner, the name of the associated workspace, and the content type. If you select **Include my changes**, the **State** column includes the date when changes were included. Search for subscriptions by keyword or filter by any of these fields. Select the **Edit** icon to make changes to a subscription’s settings.
...
Users who are assigned the [Admin role](../collaborate-share/service-roles-new-workspaces.md) in a workspace can view all subscriptions created for Power BI reports, dashboards, or paginated reports in that workspace, regardless of owner. The view provides information on the subscription name, owner, report or dashboard name, and content type. Workspace admins can also edit and take over subscriptions owned by others.
If you subscribe to reports through Power BI Organizational App, you need to install the organizational app to see the subscriptions.

> [!NOTE]
> Only the user who created subscriptions can manage subscriptions for reports and dashboards in an app or those hosted in your **My workspace**.

To access this view from within a workspace, select the gear icon :::image type="icon" source="media/end-user-subscribe/power-bi-settings-icon.png" border="false"::: > **Power BI settings** > **General** > **Notifications**.

Users with the Admin role can also make edits or take over ownership of subscriptions that aren't their own.  

For example, if the owner of a subscription is on vacation and a new recipient needs to be added to their subscription, an admin can modify the subscription by selecting the **Edit** icon and making the necessary updates to the subscription. The owner of this subscription receives an email notification that their workspace admin made an edit.  
...
Fabric administrators have control over subscriptions.
...
For more information, see [Operations available in the audit and activity logs](../admin/service-admin-auditing.md#operations-available-in-the-audit-and-activity-logs) in "Track user activities in Power BI."
...
Your Power BI admin must [enable subscriptions in your tenant](/fabric/admin/service-admin-portal-export-sharing). In addition, enable the relevant tenant switches:
...
In general, the process for subscribing to paginated reports is the same as [subscribing to reports and dashboards](#subscribe-to-a-report-or-dashboard), but there are a few differences worth noting. This section describes these differences and other considerations to keep in mind when subscribing to paginated reports.  

### Paginated reports parameters

Paginated reports let you specify the view of the report people receive in the subscription by setting the parameters in the subscription pane.  

> [!NOTE]
>
> When using query bound parameters, subscription owners should make sure that parameter values are valid. Otherwise, the subscription fails and the subscription owner gets an email stating "rsParametersNotSpecified" error.
>
> Users can choose to use "Get values from a query" when they define Available values in Report parameters in Power BI Report Builder instead of specifying values.

- Send subscriptions with either the currently selected or default parameters for your report. Set different parameter values for each subscription you create for your report.
...
1. Rerender the paginated report with the new  parameters.
...
- Custom messages are used differently in subscriptions for reports and dashboards. For reports, the message will appear above the preview in the body of the email. For dashboards, the message will appear within the generated preview, at the top of the image.
...
- The following Power BI visuals aren't supported. When you subscribe to a report containing these visuals, they display an error symbol.
  - Power BI [custom visuals](../developer/visuals/develop-power-bi-visuals.md) that aren't [certified](../developer/visuals/power-bi-custom-visuals-certified.md).
