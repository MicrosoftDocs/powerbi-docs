...
- During the Preview of dynamic per recipient subscriptions, your recipient semantic model has a limit of 1000 rows of recipients. If the recipient list exceeds 50 rows at any point, only the first 1000 recipients receive the subscription email, and the subscription creator receives an error email.
