---
title: Monitor Usage Metrics in Power BI Workspaces (preview)
description: Learn how to view, save, and analyze usage metrics for Power BI reports in workspaces. Track report views, viewers, performance, and user engagement to optimize your content strategy.
author: JulCsc
...
# Monitor usage metrics in Power BI workspaces (preview)

Monitor usage metrics in Power BI workspaces to understand how your reports and dashboards are being used across your organization. Usage metrics help you demonstrate impact, prioritize efforts, and identify which reports drive engagement. Track daily report views, discover viewer patterns, and optimize content based on actual usage data. This feedback is invaluable in guiding your work efforts.
...
1. From the workspace content list, select **More options (...)** for the report and select **View usage metrics report**.
    ...
    :::image type="content" source="media/service-modern-usage-metrics/power-bi-modern-view-usage-metrics.png" alt-text="Screenshot of steps to view Usage Metrics.":::
...
1. The first time you do this, Power BI creates the usage metrics report and lets you know when it's ready.
    ...
    :::image type="content" source="media/service-modern-usage-metrics/power-bi-modern-usage-metrics-ready.png" alt-text="Screenshot of Usage Metrics report being ready.":::

1. To see the results, select **View usage metrics**.
1. If this is the first time you've viewed a usage metrics report, Power BI might open the old usage metrics report. To display the improved usage metrics report, in the upper right corner, toggle the **New usage report** switch to **On**.

    :::image type="content" source="media/service-modern-usage-metrics/toggle-new-usage-report-on.png" alt-text="Screenshot of switching to the Usage Metrics report.":::
...
1. In the gray header bar at the top, select the **Settings** icon, then select **Settings**.

    :::image type="content" source="media/service-modern-usage-metrics/power-bi-settings-settings.png" alt-text="Screenshot of navigating to Settings menu.":::

1. Switch to the **Semantic models** tab.
1. Select the Usage Metrics Report semantic model.

    :::image type="content" source="media/service-modern-usage-metrics/power-bi-select-usage-metrics.png" alt-text="Screenshot of selecting the Usage Metrics semantic model.":::
...
1. Select the **Take over** button, then in the **Take over semantic model settings** dialog box, select **Take over** again.
1. Under **Data source credentials**, select **Edit credentials**.

    :::image type="content" source="media/service-modern-usage-metrics/power-bi-usage-metrics-edit-credentials.png" alt-text="Screenshot of editing credentials.":::

1. In the **Configure Usage Metrics Report** dialog box, select **Sign in**.

    :::image type="content" source="media/service-modern-usage-metrics/power-bi-modern-usage-metrics-configure.png" alt-text="Screenshot of signing in.":::

1. Complete the sign-in sequence and note the notification that the data source was updated successfully.

    > [!NOTE]
    > The Usage Metrics Report semantic model contains usage data for the last 30 days only. Data older than 30 days is automatically removed. It can take up to 24 hours for new usage data to be imported. You can't trigger a manual refresh by using the Power BI user interface. If you need to retain historical usage data beyond 30 days, you must export it to an external data store before it ages out of the semantic model.
...
1. In the usage metrics report, select the **File** dropdown, then select **Save a copy**.
    ...
    :::image type="content" source="media/service-modern-usage-metrics/power-bi-modern-usage-metrics-save.png" alt-text="Screenshot of saving a copy of the report.":::

1. In the **Save your report** dialog box, enter a name, then select **Save**.
...
1. The new report is saved to the **All** tab and the **Content** tab in the current workspace and added to the **Recent** list.
    ...
    :::image type="content" source="media/service-modern-usage-metrics/power-bi-modern-usage-metrics-new-report.png" alt-text="Screenshot of the new report on the Reports tab.":::
...
1. If you aren't signed in to the Power BI service, on the **File** menu select **Sign in**.
1. To connect to the Usage Metrics Report semantic model, on the **Home** ribbon select **Get Data** > **More**.
1. In the left pane, select **Power Platform**, then select **Power BI semantic models** > **Connect**.

    :::image type="content" source="media/service-modern-usage-metrics/power-bi-desktop-get-data.png" alt-text="Screenshot of getting data from Power BI semantic models.":::

1. Scroll to the desired semantic model or type _Usage Metrics_ in the search box.
1. Verify in the Workspace column that you're selecting the correct semantic model, then select **Create**.

    :::image type="content" source="media/service-modern-usage-metrics/power-bi-desktop-select-usage-metrics.png" alt-text="Screenshot of selecting the Usage Metrics Report semantic model.":::

1. Check the Fields list in Power BI Desktop, which gives you access to the tables, columns, and measures in the selected semantic model.

    :::image type="content" source="media/service-modern-usage-metrics/power-bi-desktop-fields-list.png" alt-text="Screenshot of viewing the Usage Metrics Report fields list.":::
...
1. Open the usage metrics report, select **Export** > **Analyze in Excel**.

    :::image type="content" source="media/service-modern-usage-metrics/power-bi-export-excel.png" alt-text="Screenshot of analyzing in Excel.":::
...
1. In the browser dialog asking what you want to do with the Usage Metrics report.odc file, select **Open**.
    ...
    :::image type="content" source="media/service-modern-usage-metrics/power-bi-open-odc-file.png" alt-text="Screenshot of opening the .odc file.":::

1. Power BI launches Excel. Verify the file name and path for the .odc file, then select **Enable**.

    :::image type="content" source="media/service-modern-usage-metrics/power-bi-excel-security-notice.png" alt-text="Screenshot of Excel security notice.":::
...
1. Now that Excel has opened and you have an empty PivotTable, you can drag fields onto the Rows, Columns, Filters, and Values boxes and create custom views into your usage data.
    ...
    :::image type="content" source="media/service-modern-usage-metrics/power-bi-pivottable-excel.png" alt-text="Screenshot of PivotTable in Excel.":::
...
- Duplicate reports with different Report ObjectIds in the usage metrics report can show up for the following scenarios:
  ...
  - Semantic model reinitialization
...
#### Why can't I run usage metrics on a report?
...
#### Can I retain usage metrics data for longer than 30 days?

The built-in Usage Metrics Report semantic model only retains the last 30 days of data. To preserve historical data beyond 30 days, you need to export the data periodically to an external data store. You can:

- Use [Analyze in Excel](#analyze-usage-data-in-excel) to export current data to Excel files.
- Connect to the Usage Metrics Report semantic model using Power BI Desktop and export to a data warehouse or database.
- Create a scheduled process to extract and archive the data before it ages out of the 30-day window.
...
#### Why doesn't the data seem up to date?
...
#### Why are there four reports in the previous version of the usage metrics report, but the improved version only displays three?
...
1. In the black header bar at the top, select the **Settings** icon, then select **Settings**.

    :::image type="content" source="media/service-modern-usage-metrics/power-bi-settings-settings.png" alt-text="Screenshot of Settings menu.":::

1. Switch to the **Semantic models** tab, and select the Usage Metrics Report semantic model.

    :::image type="content" source="media/service-modern-usage-metrics/power-bi-settings-usage-report-dataset.png" alt-text="Screenshot of the Usage metrics semantic model.":::

1. Copy the workspace and semantic model IDs from the URL displayed in the address bar of your browser.

    :::image type="content" source="media/service-modern-usage-metrics/power-bi-usage-metrics-url.png" alt-text="Screenshot of the Usage metrics semantic model URL.":::
...
1. Sign in to Power BI, paste the Workspace ID in the **groupId** text box and the semantic model ID into the **datasetId** text box, and then select **Run**.
    ...
    :::image type="content" source="media/service-modern-usage-metrics/power-bi-rest-api-try-it.png" alt-text="Screenshot showing Try the REST API.":::
...
1. Under the **Run** button, verify that the service returns a Response Code of **200**. That code indicates that you have successfully deleted the semantic model and its associated usage metrics reports.
    ...
    :::image type="content" source="media/service-modern-usage-metrics/power-bi-response-code-200.png" alt-text="Screenshot showing Response code 200.":::

### Create a fresh usage metrics report

1. Back in the Power BI service, you see the semantic model is gone.

    :::image type="content" source="media/service-modern-usage-metrics/power-bi-no-usage-metrics-dataset.png" alt-text="Screenshot of deleted usage metrics report.":::

1. If you still see the Usage Metrics report in the Reports list, refresh your browser.
1. Start over and [create a fresh usage metrics report](#create-and-view-a-new-usage-metrics-report).
