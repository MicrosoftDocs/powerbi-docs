...
- Due to restricted directory permissions, guest users managing app access (see [Default guest user permissions](/entra/fundamentals/users-default-permissions) for more information) may only be able to share apps with one user or group at a time. Assigning the Directory Reader role to a guest user allows them to assign multiple users to the app.
