...
> - Disabling a user in Microsoft Entra ID does not remove their access records to Power BI workspaces or other items they have access to. This is by design to avoid accidental data loss and to support recovery if the user is restored in Microsoft Entra ID. Access records remain until the user is hard deleted, and while disabled, the user cannot interact with any items.
