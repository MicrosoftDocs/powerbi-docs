...
- A cached query stored in a remote region stays in the selected region at rest as data replication is performed, but may not remain at rest in that region after data replication is completed. Other data may be processed and may go back and forth between multiple geographies.
...
- Query text and query result traffic for imported and DirectQuery data models may not transit through the home region; in scenarios such as screenshots, data alerts, report metadata, and others, the query is in the home region. The report metadata also contains queries that come from the home region, but certain DNS routing states or other network conditions may take such traffic out of the region. 
