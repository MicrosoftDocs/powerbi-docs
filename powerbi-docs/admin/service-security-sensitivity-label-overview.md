...
>* To be able to use sensitivity labels in Power BI Desktop, you must first [enable the information protection preview feature](service-security-apply-data-sensitivity-labels.md#apply-sensitivity-labels-in-power-bi-desktop-preview) and then restart the program. If the program crashes after restart, it may be because your machine lacks the required Visual C++ redistributable runtime library version. If you experience such a crash, download and install [XXX from [YYY], and then try launching Desktop again.
