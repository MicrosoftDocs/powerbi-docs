...
>* To be able to use sensitivity labels in Power BI Desktop, you must first enable the information protection feature switch and then restart the program. If the program crashes after restart, it may be because your machine lacks the required Visual C++ redistributable runtime library version. If you experience such a crash, download and install the most recent version of the library from XXX, and then try launching Desktop again.
