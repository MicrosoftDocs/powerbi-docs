...
* Currently, the only data sources supported with single sign-on (SSO) enabled are Power BI datasets, and only if the Power BI dataset doesn’t have any SSO-enabled data sources.
