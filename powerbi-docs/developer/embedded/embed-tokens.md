...
* Use an external tool, such as [Bruno](https://www.usebruno.com/downloads), to acquire a token. The request URL for a service principal must be `https://login.windows.net/{{tenantId}}/oauth2/token`, but for a master user, it can be either `https://login.windows.net/{{tenantId}}/oauth2/token` or `https://login.windows.net/common/oauth2/token`.
