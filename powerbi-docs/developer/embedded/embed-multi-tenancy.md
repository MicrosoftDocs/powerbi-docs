...
Despite these advantages, consider the scale that your *embed for your organization* app can reach in the future. For example, you might reach [limitations](#power-bi-items) around the number of Power BI items you can manage. The capacity SKU used introduces a limit on the size of memory that datasets need to fit in, how many refreshes can run at the same time and the maximum frequency of data refreshes. We recommended that you test your app when managing hundreds or thousands of datasets. It is also recommended to consider the average and peak volume of usage, as well as any specific tenants with large datasets, or different usage patterns, that are managed differently than other tenants.
