...
When designing a multi-tenant *embed for your organization* application, you must carefully choose the tenancy model that best fits your needs. A tenancy model determines how each tenant's data is mapped and managed within Power BI. Your tenancy model impacts application design and management. Switching to a different model later may become costly and disruptive.

This article describes the two different approaches and analyzes them according to several evaluation criteria. The *workspace isolation* solution, is described on the left side of the article, and the *row-level security isolation* solution is described on the right side of the article. You can also view a [summary](#comparison-summary) of this evaluation, at the end of the article.
...
With Power BI workspace-based isolation, the *embed for your organization* app supports multiple tenants from a single Power BI tenant. Workspace-based isolation contains all the Power BI content that different tenants use. The separation of tenants is done at the Power BI workspace level, by creating multiple workspaces. Each workspace contains the relevant datasets, reports, and dashboards for that tenant. Also, each workspace is connected only to that tenant's data. If you need additional isolation, you can create a *master user* or a *service principal* for each workspace and its content.

:::column-end:::
:::column span="":::

With row-level security-based isolation, the *embed for your organization* app uses a single workspace to host multiple tenants. Each Power BI item (such as reports, dashboards and datasets) is created once and is used by all tenants. Data separation between tenants is accomplished using [row-level security](embedded-row-level-security.md) on the multi-tenant dataset. When end users log into the app and open content, an embed token is generated for that user's session, with the roles and filters that ensure that users only see the data they're permitted to see. If users from the same tenant are not permitted to view the same data, the application developer needs to implement hierarchical roles both between tenants and within the same tenant.
...
Despite these advantages, consider the scale that your *embed for your organization* app can reach in the future. For example, you might reach [limitations](#summary-comparison-of-the-different-approaches) around the number of Power BI items you can manage. The capacity SKU used introduces a limit on the size of memory that datasets need to fit in, how many refreshes can run at the same time and the maximum frequency of data refreshes. We recommended that you test your app when managing hundreds or thousands of datasets. It is also recommended to consider the average and peak volume of usage, as well as any specific tenants with large datasets, or different usage patterns, that are managed differently than other tenants.
...
* The maximum dataset size in a capacity is 10 GB.
