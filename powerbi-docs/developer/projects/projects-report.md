...
- As PBIR components are stored as sub-folders and files, they are subject to the Windows operating system limits on folder + file name length (default 260 characters). In particular, long Power BI table names may break this limit, resulting in an error when saving in PBIR format. Mitigate this risk by using a short folder path as the home for your PBIR solution, and save immediately after creating or renaming tables.
