...
**Enable** - Saving as a Power BI Project is currently in **PREVIEW**. Before giving it a try, you must first enable it in **Preview features**. To enable, in Power BI Desktop > **File** > **Options and settings** > **Options** > **Preview features**, select the checkbox for **Power BI Project (.pbip) save option**.
...
This is the dataset item folder containing a collection of files and folders that represent a Power BI dataset. The dataset folder contains some of the most important files you're likely to work on, like model.bim. To learn about sub-folders and files, see [Project dataset folder](projects-dataset.md).

##### \<project name>.Report

The Report item folder is a collection of files and folders that represent a Power BI Report. To learn about sub-folders and files, see [Project report folder](projects-report.md).
...
Most metadata files are JSON. JSON schema is used for validation and documentation.

Schemas allow developers to:

- Learn about properties allowed to be configured.
- Validate their changes with in-line JSON validation provided by code editor.
- Improve editing experience with syntax highlighting and autocompletion by code editor.
- Implement external tools with knowledge about all the supported properties within the metadata files.
