...
If you're a data services provider and have created a template app to help your customers get started with their data on your service, you can make it easier for them to install your template app by automating the configuration of your template app's parameters.
...
This customer experience is illustrated here.
...
>[!Note]
>While parameter values are configured by the ISV in the process of creating the install ticket, data source-related credentials are only supplied by the user in the final stages of the installation. This prevents them from being exposed to a third party, thus ensuring a secure connection between the user and the template app data sources.

## Prerequisites

Before you get started, you need to have:

* Your own Azure Active Directory (Azure AD) tenant setup. For instructions how to set one up, see [Create an Azure Active Directory tenant](https://docs.microsoft.com/power-bi/developer/embedded/create-an-azure-active-directory-tenant).
...
* *App ID*, *Package Key*, and *Owner ID* as they appear in the installation URL at the end of the [Define the properties of the template app](../../connect-data/service-template-apps-create.md#define-the-properties-of-the-template-app) process when the app was created.
    ...
    You can also get the same link by selecting **Get link** in the template app's [Release Management](../../connect-data/service-template-apps-create.md#manage-the-template-app-release).
...
>[!NOTE]
>You can test your preconfigured installation application on your template app if the template app is ready for installation, even if it isn't publicly available on AppSource yet. However, for users outside your tenant to be able to use the automated installation application to install your template app, the template app must be publicly available in the [Power BI Apps marketplace](https://app.powerbi.com/getdata/services). So before distributing your template app using the automated installation application you're creating, be sure to publish it to the [Partner Center](https://docs.microsoft.com/azure/marketplace/partner-center-portal/create-power-bi-app-offer).


## Install and configure your template app

In this section, you'll use an automated installation Azure Function sample that we created to preconfigure and install your template app. This sample has deliberately been kept simple for demonstration purposes. It allows you to use an [Azure Function](https://docs.microsoft.com/azure/azure-functions/functions-overview) and [Azure App Configuration](https://docs.microsoft.com/azure/azure-app-configuration/overview) to easily deploy and use the automated installation API for you template apps.
...
### Download the automated installation Azure Function sample

Download the [automated installation Azure Function sample](https://github.com/microsoft/Template-apps-examples/tree/master/Developer%20Samples/Automated%20Install%20Azure%20Function) from GitHub to get started.

![Automated installation Azure Function sample](media/template-apps-auto-install/azure-function-sample.png)

### Set up your Azure App Configuration

To run this sample, you need to set up your Azure App Configuration with the values and keys as described below. The keys are the **Application ID**, the **Application secret**, and your template app's **AppId**, **PackageKey**, and **OwnerId**. See the following sections for information about getting these values. 
...
Fill in all relevant Template app properties as they are defined when the app is created. These properties are the template app's **AppId**, **PackageKey**, and **OwnerId**.
...
1. Open the **Release Management** pane.
    ...
    ![Screenshot of Release Management pane.](media/template-apps-auto-install/release-management-001.png)
...
1. In the left pane, select **All services** > **App registrations**.
    ...
    ![Screenshot of App registration search](media/template-apps-auto-install/embed-sample-for-customers-003.png)

1. Select the application that needs the **applicationId**.

    ![Screenshot of choosing an app](media/template-apps-auto-install/embed-sample-for-customers-006.png)

1. There's an **Application ID** that's listed as a GUID. Use this **Application ID** as the **applicationId** for the application.

    ![Screenshot of applicationId](media/template-apps-auto-install/embed-sample-for-customers-007.png)
...
1. Select the application that needs to use the **ApplicationSecret**.
    ...
    ![Screenshot of choosing an app](media/template-apps-auto-install/embed-sample-for-customers-0038.png)
...
1. Enter a name in the **Description** box, and select a duration. Then select **Save** to get the **Value** for your application. When you close the **Keys** pane after you save the key value, the value field shows only as hidden. At that point, you aren't able to retrieve the key value. If you lose the key value, create a new one in the Azure portal.
    ...
    ![Screenshot of Key value](media/template-apps-auto-install/embed-sample-for-customers-042.png)
...
1. The ```POST /api/install``` request is issued to your Azure function. The request body consists of key-value pairs, where the key is the parameter name and the value is the desired value to be set.
