...
This blocking is in effect only for the current session. So if a user blocks the CV modal dialogs but later changes their mind, they can re-enable the dialogs by starting a new session (refreshing the reports page in Power BI service, or exiting and restarting Power BI Desktop).
