...
**Dataflows** are designed to use the **Common Data Model**, a clearly defined schema of common business entities, so you can go from data sources to Power BI dashboards with nearly zero friction. When your dataflows are built using the well-defined entity shapes and semantics of the Common Data Model, you’ll gain the benefits of a known data structure, regardless of where your data originates, allowing you to create dashboards and reports quickly and with ease.
...
* Organizations can map their data to standard Common Data Model entities or define their own custom entities. These entities can then be used as building blocks to create reports, dashboards, and apps that work out of the box, and distribute them to users across their organization. 
...
For organizations that want to extend the Common Data Model (CDM), dataflows enable business intelligence professionals to customize Common Data Model entities, or create new ones. This approach, considered a self-service approach customizing the CDM data model, can then be used with dataflows to build customized apps, which in turn generate customized Power BI dashboards that are tailored to their organization.
...
Power BI can also connect to Common Data Model compliant folders created by other Azure services that are stored in your organization’s Azure Data Lake Storage account, and analysts can work seamlessly with this data in Power BI. 
...
By storing business data in a standardized **Common Data Model** format, your BI Pros (or developers) can create apps that generate quick, easy, and automatic visuals and reports. Those include, but aren’t limited to:


* Mapping your data to standard Common Data Model entities to unify data and leverage the known schema to drive out-of-the-box insights
