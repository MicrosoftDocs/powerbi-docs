...
If you're using [centralized datasets](../service-datasets-across-workspaces.md) that are used across the organization, we recommend that you create two types of workspaces:
...
>[!NOTE]
>If your organization is using [shared centralized datasets](../service-datasets-share.md), you can skip this recommendation.
...
If you want to manage the version history of your reports and datasets, use [Power BI's autosync with OneDrive](../service-connect-to-files-in-app-workspace-onedrive-for-business.md). This will keep your files updated with the latest version. It will also enable you to retrieve older versions if needed.
...
You can separate modeling development from visualizations, by managing a [shared dataset](../service-datasets-share.md) in an external workspace, using XMLA r/w capabilities. The shared dataset can connect to multiple reports in various workspaces that are managed in multiple pipelines.
...
You also need to [monitor the load on the capacity](../service-admin-premium-monitor-capacity.md), so that you can catch extreme loads before they reach production.  
