---
title: "Copilot in Power BI: Integration Overview and Benefits"
description: "Discover how Copilot in Power BI enhances data analytics by assisting with report creation and data exploration. Learn how to enable, use, and govern Copilot experiences for semantic models and reports."
author: JulCsc
...
Copilot in Microsoft Fabric is a generative AI assistant that enhances the data analytics experience in the Fabric platform. Each Fabric workload, including Power BI, offers different Copilots. Various Power BI personas, including enterprise developers, self-service users, and business users, can use the different Copilot in Power BI experiences. This article provides an in-depth overview of how Copilot in Power BI enhances the data analytics experience by assisting users in creating and consuming semantic models and reports.

> [!NOTE]
> For more information, see the [Copilot in Power BI documentation](copilot-introduction.md).

:::image type="content" source="media/copilot-integration/copilot-power-bi-users.svg" alt-text="Diagram showing how different Power BI users can use Copilot for semantic models and reports.":::

Copilot in Power BI helps both Power BI developers and analysts *create* models and reports, while also giving business users new ways to *consume* those models and reports. The following diagram shows how different personas in Power BI can use the various Copilot experiences to create or consume their data.

:::image type="content" source="media/copilot-integration/copilot-creation-consumption.svg" alt-text="Diagram showing Copilot in Power BI creation and consumption experiences for different audiences.":::
...
Copilot in Power BI provides new opportunities and approaches to work with your data. However, to get the most value from these new opportunities and approaches, you need a healthy data culture and good adoption of Power BI. This culture includes preparing effective user training and enablement, as well as monitoring and governing Copilot usage. Enabling and using Copilot in Power BI requires careful deliberation. You can't just turn on Copilot and expect to see productivity enhancement across your organization.
...
:::image type="content" source="media/copilot-integration/copilot-experiences-diagram.png" alt-text="Diagram showing how Copilot in Power BI experiences differ in input, grounding, and output.":::

In summary, several areas of the Copilot architecture differ depending on which Copilot experience you use.

- **Input:** Different experiences require users to provide input in different ways.
  - The *ask data questions* experiences for models or reports require users to write a prompt in the Copilot chat pane.
  - The *generate measure descriptions* experience requires users to push a button in the Properties pane in the Model view of Power BI Desktop.
...
- **Postprocessing:** Depending on the experience, Copilot handles the large language model (LLM) response in different ways. Examples of postprocessing differences between the Copilot experiences in Power BI include:
  - The *DAX query view* experience runs DAX from the LLM response through a DAX parser to ensure the generated DAX query runs. If not - for instance, if the LLM produces hallucinations - Copilot requests a new LLM response.
...
- **Output:** Depending on the experience, Copilot returns different types of outputs to a user after postprocessing. Examples of output differences between the Copilot experiences in Power BI include:
...
### Copilot in Power BI usage process flow
...
:::image type="content" source="media/copilot-integration/copilot-use-process-flow.svg" alt-text="Diagram showing the process flow for using Copilot in Power BI across mobile, Desktop, and workspaces.":::

The diagram consists of the following parts and processes:

| **Item** | **Description** |
|---|---|
| **1** | The user provides an input to Copilot, which could be a written prompt or another interaction that generates a prompt. All interactions with Copilot are user specific. The user can interact with Copilot in Power BI either by using the Power BI mobile app, Power BI Desktop, or in a Fabric workspace. |
| **2** | In the Power BI mobile app, users can access a standalone Copilot experience from the app home screen on phones and tablets to chat with data without first opening a specific report. Users can generate summaries, key insights, and auto-generated visuals with citations, share results with colleagues, and use voice dictation for prompts on iOS. Users can also view reports in Power BI apps or Fabric Org Apps and ask questions about specific reports by using the in-report Copilot. For more information, see [Overview of Copilot in Power BI mobile apps](../explore-reports/mobile/mobile-copilot-overview.md). |
| **3** | In Power BI Desktop, users can use Copilot while they develop semantic models. They can ask data questions, generate DAX queries in the DAX query view, or generate measure descriptions for the semantic model.<br><br>To use Copilot in Power BI Desktop, users need to connect to a supported workspace on Fabric capacity. |
| **4** | In a workspace that uses a workspace type of a supported Fabric capacity, users can use Copilot with semantic models and reports that they publish from Power BI Desktop, upload via Git integration, or create in Fabric. Users might also use notebooks to help develop and manage semantic models by using semantic link and could use Copilot in those notebooks (even though notebooks aren't an item of the Power BI workload). |
...
| **6** | Workspaces that have a workspace type of Power BI Pro, Premium Per User (PPU), or an unsupported F SKU don't support Copilot.<br><br>Users can consume items from these workspaces in Power BI Desktop and use Copilot, as long as they set up Power BI Desktop to consume Fabric capacity from a supported workspace.<br><br>Users can also create reports in these workspaces, and use Copilot in these reports, as long as that report connects to a semantic model in supported workspace. |
...
To use Copilot in Power BI, first enable it. The steps to enable Copilot in Power BI are similar to the steps you take to [enable Copilot in Fabric](/fabric/fundamentals/copilot-enable-fabric). You can enable or disable Copilot specifically for Power BI, like with the other workloads.

Consider these scenarios:

- **P SKUs:** You can also use Copilot in Power BI if you have a P SKU.
- **Cross-workspace consumption:** Some Copilot experiences still work when you consume items located in workspaces that use a workspace type that doesn't support Copilot, like Pro, PPU, or an unsupported SKU. For example, you can use Copilot in Power BI Desktop while consuming a semantic model published to workspaces that don't support Copilot. Similarly, you can also connect to these semantic models from certain Fabric items, like notebooks using the Semantic Link feature.
- **Cross-capacity consumption:** Fabric administrators can [delegate Copilot consumption to a Fabric Copilot capacity (FCC)](/fabric/enterprise/fabric-copilot-capacity). This delegation can prevent Copilot usage from impacting your main Fabric capacity that other Fabric workloads and items need to support business-critical processes or decision making.
...
Power BI offers different Copilot experiences that you can use with either semantic models or reports. The following articles in this series describe these experiences in detail and how you can best use them.

- [Use Copilot with semantic models](copilot-semantic-models.md)
- [Use Copilot with Power BI reports](copilot-reports-overview.md)
- [Copilot for Power BI apps](copilot-apps-overview.md): Report-scoped and app-scoped AI features that help you summarize reports, find items, and answer data questions within Power BI apps.
- [Standalone Copilot experience in Power BI](../explore-reports/copilot-chat-with-data-standalone.md): A full-screen Copilot experience in the Power BI service that finds and answers questions about any report, semantic model, or Fabric data agent you have access to.
