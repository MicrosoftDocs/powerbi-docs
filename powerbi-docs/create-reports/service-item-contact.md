...
The contact list information is also used in other places. For example, it appears in some error scenarios in the error dialog box. Automated email messages related to the item, like access requests, are sent to the contact list.
...
1. Locate the desired item, select **More options (...)** > **Settings**.
...
1. Locate the **Contact** input field and set a value.
...
* Why do we recommend listing only one contact? If a dashboard or report has multiple contacts, when someone opens the report header and selects the link to email the contacts, some email clients separate the names with commas instead of semicolons. The resulting email address doesn't work.
