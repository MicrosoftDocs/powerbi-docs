...
description: This article describes DirectQuery reporting scenarios by using the Optimize ribbon in Power BI Desktop
author: julcsc
...
In this article, we go through several DirectQuery report authoring scenarios showing how you can streamline your experience with the **Optimize** ribbon in Power BI Desktop. With the Optimize ribbon, you can control when visuals refresh with the **Pause visuals** and **Refresh visuals** features, use **Optimization presets** to quickly apply predefined settings, and easily launch **Performance analyzer** to go deeper into the queries themselves, including the SQL queries DirectQuery generates. We also show how you can edit relationships right in Model view's Properties pane *without* running queries to preview or validate the data.
...
Just like with the previous scenario, in the **Optimize** ribbon, we select **Pause visuals** to have the report enter a paused state. Normally, in a DirectQuery report, anytime a new measure is created, all the visuals on the report page refresh. But, with the report in a paused state, we can create new measures without queries being sent. The visuals are instead in a pending state. We can add the measures to the pending visuals. We can then refresh an individual visual to see results with the new measures or simply resume visual queries when done.

**Pause visuals when creating new measures**:
...
If we select the **Optimization presets** dropdown in the **Optimize** ribbon, and then select **Query reduction** for our report, a dialogue appears describing the settings that are applied.
...
**Set the Interactivity optimization preset**:
...
You can contribute too! With Microsoft Learn, Power BI documentation like this article is open-source. To learn more about how you can contribute, be sure to check out the [Microsoft Learn documentations contributors guide](/contribute/).
