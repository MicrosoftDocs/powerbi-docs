...
|Hierarchies  | Logical groupings  | Establish clear hierarchies within the data, especially for dimension tables that could be used to drill down in reports.  | In table context menu, select New hierarchy | In the `Date` table, create a hierarchy: `Year` > `Quarter` > `Month` > `Day`. In `Geography` table: `Country` > `State` > `City`. |
