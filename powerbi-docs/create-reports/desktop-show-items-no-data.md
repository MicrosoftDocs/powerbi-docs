...
To illustrate how Power BI determines which data is relevant to display, see the following example. Using the model represented in the [example data model](#example-data-model) section, found at the end of this article, consider a table built with the following settings:
...
Now look at a different combination:
...
![Screenshot of the fields menu, highlighting Show items with no data from the Remove field dropdown menu.](media/desktop-show-items-no-data/show-items-no-data-05.png)
