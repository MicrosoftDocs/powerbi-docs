...
:::image type="content" source="media/desktop-set-visual-query-limits/desktop-set-visual-query-limits-00.png" alt-text="Screenshot of the error message displayed when a visual query reaches capacity limits.":::
...
1. Scroll to the **Query limit simulations** section. 
   :::image type="content" source="media/desktop-set-visual-query-limits/desktop-set-visual-query-limits-01.png" alt-text="Screenshot of the options menu where the query limit simulations section is available.":::
1. Using the dropdown menu, select a capacity to simulate. You can choose from:
   - Auto (recommended)
   - Shared capacity
   - Premium capacity
...
By default, new reports have "auto (recommended)" selected. With auto limits, Power BI does its best to identify where the model is hosted and apply those limits. Otherwise, it defaults to the Shared capacity limits to ensure that queries that complete on Desktop complete for published reports, regardless of where the report's data ends up hosted. Below the dropdown, you can see the exact timeout and memory limits currently being applied for the chosen capacity. 
...
- Limits defined using this report setting apply to all queries sent from the report view. Queries sent from the data or model views aren't affected.
- With live connected reports, you may notice visuals erroring before they reach query limits you set. This might occur if the external Analysis Services engine to which your report is live connected has its own stricter query limits. Even when Power BI Desktop applies query limits through the report setting described in this article, queries are still subject to any other limits imposed by the data source.
