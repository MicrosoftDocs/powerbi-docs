...
- **Optimize Microsoft Fabric Data Warehouse sources**: For DirectQuery models that use Microsoft Fabric Data Warehouse or the SQL analytics endpoint, optimize the source for the query patterns generated by Power BI visuals. For more information, see [Performance guidelines for Fabric Data Warehouse](/fabric/data-warehouse/guidelines-warehouse-performance).
  - **Use data clustering for large filtered tables**: For large fact tables that are frequently filtered by slicers, date ranges, tenant, customer, geography, or other selective columns, consider using data clustering on those columns. Data clustering can improve performance by enabling file and row group skipping and reducing the amount of data scanned. For more information, see [Data clustering in Fabric Data Warehouse](/fabric/data-warehouse/data-clustering).
  - **Maintain statistics on report query columns**: Create or update statistics on columns commonly used by Power BI in filters, joins, grouping, and ordering. Accurate statistics help the query optimizer choose efficient plans for interactive report queries. For more information, see [Statistics in Fabric Data Warehouse](/fabric/data-warehouse/statistics).
  - **Maintain healthy table storage for SQL analytics endpoint queries**: When Power BI queries Lakehouse tables through the SQL analytics endpoint, regularly compact and optimize Delta tables to avoid excessive small files, inefficient row groups, and unnecessary storage scans. For more information, see [Cross-workload table maintenance and optimization](/fabric/fundamentals/table-maintenance-optimization).
