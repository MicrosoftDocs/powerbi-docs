...
- Your model could be a DirectQuery model, but you want to boost performance. In a Composite model, performance can be improved by configuring appropriate storage for each table. You can also add [aggregations](../enterprise/aggregations-auto.md). Both of these optimizations are discussed later in this article.
...
A Composite model can be optimized by configuring table [storage modes](../transform-model/desktop-storage-mode.md), and by adding [aggregations](../enterprise/aggregations-auto.md).
...
- [Aggregations in Power BI Desktop](../enterprise/aggregations-auto.md)
