...
This usage scenario focuses on *advanced data model management*, which is when a Power BI content creator uses a third-party tool to broadens the capabilities for designing, managing, and debugging data models. Use of a third-party tool can occur during authoring by using the [external tools](https://docs.microsoft.com/en-us/power-bi/transform-model/desktop-external-tools) capability in Power BI Desktop, or it can occur after it's published by connecting to a data model using its [XMLA endpoint](../enterprise/service-premium-connect-tools.md). Models are hosted in either the Power BI service, Azure Analysis Services, or SQL Server Analysis Services. This scenario focuses on using the XMLA endpoint in the Power BI service.
