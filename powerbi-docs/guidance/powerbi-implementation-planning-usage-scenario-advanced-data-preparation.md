...
Investing in data preparation, and centralizing data preparation efforts, seek to achieve the following objectives:
...
The *advanced data preparation* usage scenario expands on the [self-service data preparation](powerbi-implementation-planning-usage-scenario-self-service-data-preparation.md) scenario. Its objective is to increase dataflow reuse by multiple users across various teams and for a variety of use cases.

Separate workspaces, organized by dataflow purpose, are particularly helpful when dataflow output is provided to multiple dataset creators, especially when they are on different teams in the organization. Separate workspaces are also helpful for managing security roles when different people manage dataflows versus consume dataflows.
...
The advanced data preparation scenario is one of the self-service BI scenarios. However, a centralized team member can use this technique in a similar way to what's described in the [managed self-service BI](powerbi-implementation-planning-usage-scenario-managed-self-service-bi.md) scenario. For a complete list of the self-service scenarios, see the [Power BI usage scenarios](powerbi-implementation-planning-usage-scenario-overview.md) article.
...
| ![Item 3.](media/common/icon-03-red-30x30.png) | Dataflows are developed by using Power Query Online, which is a [web-based version of Power Query](/power-query/power-query-what-is-power-query#where-can-you-use-power-query). |
...
| ![Item 7.](media/common/icon-07-red-30x30.png) | One or more additional workspaces exist that are intended to provide access to the final dataflow, which delivers production-ready data to data models. |
...
| ![Item 9.](media/common/icon-09-red-30x30.png) | Dataset creators (who consume the dataflow output) have viewer access to the workspace that contains the final dataflow output. The dataflow creators also have access to manage and publish content in the workspace (not depicted in the scenario diagram). |
| ![Item 10.](media/common/icon-10-red-30x30.png) | All of the workspaces involved have their [license mode](../collaborate-share/service-create-the-new-workspaces.md#premium-capacity-settings) set to **Premium per user**, **Premium per capacity**, or **Embedded**. These license modes allow for the use of linked and computed tables across workspaces, which are required in this scenario. |
...
A [dataflow](../transform-model/dataflows/dataflows-create.md) comprises a collection of tables (also known as entities). Each table is defined by a query, which contains the data preparation steps required to populate the table with data. All work to create a dataflow is done in [Power Query Online](https://powerquery.microsoft.com/). A dataflow can be created in [multiple products](/power-query/dataflows/overview-dataflows-across-power-platform-dynamics-365#dataflows-can-be-created-in-multiple-microsoft-products), including Power Apps, Dynamics 365 Customer Insights, and Power BI.
...
Use of *composable building blocks* is a design principle that allows system components to be managed, deployed, secured, and used in various combinations. Creating modular, self-contained dataflows that are specific to a purpose is a best practice and helps to achieve data reuse and enterprise scale. Modular dataflows are also easier to manage and test.

Three types of dataflows are shown in the scenario diagram: staging dataflow, transformation dataflow, and final dataflow.

#### Staging dataflow

A [staging dataflow](/power-query/dataflows/best-practices-for-data-warehouse-using-dataflows#staging-dataflows) (sometimes called a *data extraction dataflow*) copies raw data as-is from the source. Having the raw data extracted, with very few transformations applied, means that downstream transformation dataflows (described next) can use the staging dataflow as their source. This modularity is particularly useful when:

- Access to a source is highly limited to narrow time windows and/or to a few users.
- Temporal consistency is desired to ensure that all downstream dataflows (and related datasets) deliver data that was extracted from the source at the same time.
...
- A copy of the source system data is useful for reconciliation processes and data quality verifications.
...
A transformation dataflow includes all the common transformation steps required to prepare and restructure the data. However, there's still a focus on reusability at this layer to ensure the dataflow is suitable for multiple use cases and purposes.

#### Final dataflow

A final dataflow represents the prepared output. Some additional transformations may occur based on the use case and purpose. For analytics, a [star schema](/power-query/dataflows/best-practices-for-data-warehouse-using-dataflows#build-a-star-schema) is the preferred format of the final dataflow.

Computed tables are visible to data modelers that are granted the workspace viewer role. This technique is described further in the [types of dataflow tables](#types-of-dataflow-tables) topic below.

> [!TIP]
> Data lakes are often designed with *zones* (like bronze, silver, and gold). The three types of dataflows previously described represent a similar design pattern. To make the best possible data architecture decisions, give thought to who will maintain the data, the expected use of the data, and the skill level required by people accessing the data.

### Workspaces for dataflows

If all dataflows are created in a single workspace, that would significantly limit the extent of reusability. Using a single workspace also greatly limits the security options available when supporting multiple types of users across teams and/or for different use cases. We recommend using [multiple workspaces](/power-query/dataflows/best-practices-reusing-dataflows#use-multiple-workspaces) to provide better flexibility when supporting self-service creators from various areas of the organization.

The two types of workspaces shown in the scenario diagram include:

- **Workspace 1:** The purpose of the first workspace is to [centrally manage dataflows](../transform-model/dataflows/dataflows-develop-solutions.md#create-user-dataflows-with-security-applied) (sometimes referred to as a *backend workspace*). This workspace contains both the staging and transformation dataflows because they're managed by the same people. The dataflow creators are often from a centralized team, such as IT, BI, or the Center of Excellence. The dataflow creators should be assigned to the workspace admin, member, or contributor role.
- **Workspace 2:** The purpose of the second workspace is to [deliver the final dataflow output](../transform-model/dataflows/dataflows-develop-solutions.md#create-user-dataflows-with-security-applied) to consumers of the data (sometimes referred to as a *user workspace*). Dataset creators are often self-service analysts, power users, or citizen data engineers. They should be assigned to the workspace viewer role as they only need to [consume the output](/power-query/dataflows/best-practices-reusing-dataflows#set-the-correct-access-levels-on-workspaces) of the final dataflow. To support the dataset creators from various areas of the organization, it's possible that you'll create numerous workspaces like this one, based on use case and security needs.

> [!TIP]
> We recommend reviewing ways to [support dataset creators](powerbi-implementation-planning-usage-scenario-self-service-data-preparation.md#support-dataset-creators) as discussed in the [self-service data preparation](powerbi-implementation-planning-usage-scenario-self-service-data-preparation.md) scenario. It's important to understand that dataset creators can still use the full capabilities of Power Query within Power BI Desktop. They can optionally add additional query steps to transform the dataflow data or merge the dataflow output with other sources.

### Types of dataflow tables

Three types of dataflow tables (also known as entities) are depicted in the scenario diagram.

- **Standard tables:** A standard table queries an external data source, such as a database. In the scenario diagram, standard tables are depicted in the staging dataflow.
- **Linked tables:** A [linked table](/power-query/dataflows/linked-entities) references a table from another dataflow. A linked table doesn't duplicate the data. Rather, it allows the reuse of a standard table multiple times for multiple purposes. Linked tables aren't visible to workspace viewers since they inherit permissions from the original dataflow. In the scenario diagram, linked tables are depicted twice:
...
- **Computed tables:** A [computed table](/power-query/dataflows/computed-entities) performs additional computations by using another dataflow as its source. Computed tables allow customizing the output as needed for individual use cases. In the scenario diagram, computed tables are depicted twice:
  ...
  - In the final dataflow for delivering output to dataset creators. Since computed tables persist the data again (after the dataflow refresh), data modelers can access the computed tables in the final dataflow with the workspace viewer role.

> [!NOTE]
> There are many design techniques, patterns, and [best practices](../transform-model/dataflows/dataflows-best-practices.md) that can take dataflows from self-service to enterprise-ready. Also, dataflows in a workspace that its license mode set to **Premium per user** or **Premium capacity** can use [advanced features](../transform-model/dataflows/dataflows-premium-features.md). Linked and computed tables (also known as entities) are two advanced features that are essential for increasing the reusability of dataflows.
...
- Use the same workspace for the staging and transformation dataflows.
- Put complex operations that can [query fold](/power-query/power-query-folding) early in the Query Editor steps. The goal is to achieve the best performance.
- Use [incremental refresh](../transform-model/dataflows/dataflows-understand-optimize-refresh.md) for a dataflow to optimize larger data volumes.
...
> [!NOTE]
> There's another type of dataflow called a [streaming dataflow](../dataflows/dataflows-streaming.md) (not depicted in the scenario diagram). A streaming dataflow is a technique to achieve real-time analytics in Power BI. Streaming dataflows aren't included in the advanced data preparation scenario.

### Azure Data Lake Storage Gen2

In Microsoft Azure, an ADLS Gen2 account is a specific type of Azure Storage account that has the [hierarchical namespace](/azure/storage/blobs/create-data-lake-storage-account) enabled. ADLS Gen2 has [performance, management, and security advantages](/azure/storage/blobs/data-lake-storage-introduction) for operating analytical workloads. By default, Power BI dataflows use internal storage, which is a built-in data lake account managed by the Power BI service. Optionally, organizations may *bring their own data lake* by connecting to an organization's ADLS Gen2 account.
...
When using linked and computed tables, make sure that each workspace is assigned to the same ADLSGen2 storage account.
...
- **Workspace-level storage:** A Power BI administrator can configure a setting to allow workspace-level storage permissions. When enabled, this setting allows [workspace administrators to use a different storage account](../transform-model/dataflows/dataflows-azure-data-lake-storage-integration.md#connecting-to-an-azure-data-lake-gen-2-at-a-workspace-level) than the one defined at the tenant-level. Enabling this setting is particularly helpful for decentralized business units that manage their own data lake in Azure.
...
When working with dataflows, a data gateway is required when:
...
> [!TIP]
> Dataflows are only supported using a centralized [data gateway](../connect-data/service-gateway-personal-mode.md#on-premises-data-gateway-vs-on-premises-data-gateway-personal-mode) in *standard mode*. A gateway in *[personal mode](../connect-data/service-gateway-personal-mode.md)* isn't supported when working with dataflows.

### System oversight

The [activity log](../admin/service-admin-auditing.md) records user activities that occur in the Power BI service. Power BI administrators can use the activity log data that's collected to perform [auditing](powerbi-adoption-roadmap-system-oversight.md#auditing) to help them understand usage patterns and adoption. The activity log is also valuable for supporting governance efforts, security audits, and compliance requirements. In the advanced data preparation scenario, the activity log data is particularly helpful to track the management and use of dataflows.
