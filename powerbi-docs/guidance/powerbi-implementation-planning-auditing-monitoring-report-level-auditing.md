...
This article focuses on auditing and monitoring of reports and visuals. However, achieving good performance for a report and its visuals isn't possible when the underlying semantic model and/or data source doesn't perform well. For information about auditing and monitoring of semantic models and dataflows, see [Data-level auditing](powerbi-implementation-planning-auditing-monitoring-data-level-auditing.md).
...
- Before [endorsing](../collaborate-share/service-endorse-content.md) a report (particularly when it's to be certified).
- Prior to [publishing](../create-reports/desktop-upload-desktop-files.md) a report to a production workspace.
- When including a report in a [Power BI app](../collaborate-share/service-create-distribute-apps.md).
...
The main auditing resource available to report creators are the [usage metrics](../collaborate-share/service-modern-usage-metrics.md) reports, which are built into the Power BI service.
...
The usage metrics reports are available to report creators and owners who are assigned to the Contributor, Member, or Admin [workspace role](../collaborate-share/service-roles-new-workspaces.md). To make the usage metrics reports visible to workspace viewers (content consumers), you can [create a copy of the usage report](../collaborate-share/service-modern-usage-metrics.md#create-a-copy-of-the-usage-report) and customize it.
...
The _ViewUsageMetrics_ operation in the [Power BI activity log](../enterprise/service-admin-auditing.md) allows Power BI administrators to monitor which content creators and owners are using the usage metrics reports. You can use that information to guide training and documentation efforts.
...
> - Table [storage mode(s)](../transform-model/desktop-storage-mode.md)
> - Whether a [data gateway](../connect-data/service-gateway-deployment-guidance.md) is used
> - Whether a [Power BI Premium](../enterprise/service-admin-premium-manage.md) capacity is involved
> - Whether [query caching](../connect-data/power-bi-query-caching.md) is enabled
...
> - The number of [active connections](../connect-data/desktop-directquery-about.md#maximum-number-of-connections-option-for-directquery)
...
Some organizations have specific requirements for [endorsed](../collaborate-share/service-endorse-content.md) (certified or promoted) reports. That's particularly true for reports that are widely used throughout the organization. In that case, you might be required (or encouraged) to verify Performance Analyzer results before publishing the report, or before it's certified.
