...
> [!IMPORTANT]
> Access policies and session policies operate in real time. Other policy types in Defender for Cloud Apps involve a short delay in alerting. Most other types of DLP and auditing also experience latency, including [DLP for Power BI](/fabric/governance/data-loss-prevention-respond) and the [Power BI activity log](../enterprise/service-admin-auditing.md).
...
You can set up an activity policy to check events recorded in the [Power BI activity log](../enterprise/service-admin-auditing.md). The policy can act on a single activity, or it can act on repeated activities by a single user (when a specific activity occurs more than a set number of times within a set number of minutes).
