...
> [!TIP]
> The Power BI [activity log](../enterprise/service-admin-auditing.md) includes an event that records when a user signs up for a [trial license](../fundamentals/service-self-service-signup-purchase-for-power-bi.md#start-a-power-bi-individual-user-trial). You can combine that event with the user license data (sourced from Microsoft Entra ID) to produce a complete picture.
