...
- **Accumulating history**: Because the Power BI activity log provides up to 28 days of data, creating your entire auditing solution involves accumulating a set of files over time that store all historical events. Accumulating historical files is simpler to accomplish with other tools.
...
For more information about what dimension tables and [fact tables](star-schema.md#fact-tables) you might include in your star schema, see [Create a data model](#create-a-data-model) later in this article.
...
| Programmatic | Power BI activity log (API or PowerShell cmdlet) | 28 | | :::image type="content" source="../includes/media/yes-icon.svg" alt-text="Power BI activity log (API or PowerShell cmdlet) is a good choice for automated auditing processes." border="false"::: | | :::image type="content" source="../includes/media/yes-icon.svg" alt-text="Power BI activity log (API or PowerShell cmdlet) is a good choice to get started quickly." border="false"::: |
...
> - **Update admin API tenant setting in Fabric**: In the Fabric admin portal, add the security group to the _Allow service principals to use read-only Power BI admin APIs_ tenant setting.
...
> - **Update the detailed metadata tenant settings in Power BI**: Optionally, in the Fabric admin portal, add the security group to the _Enhance admin API responses with detailed metadata_ tenant setting and also the _Enhance admin API responses with DAX and mashup expressions_ tenant setting.
...
### Set Fabric tenant settings

There are several tenant settings in the Fabric admin portal that are relevant for extracting tenant-level auditing data.
...
**Checklist** - When setting up the Fabric tenant settings, key decisions and actions include:
...
- **How much history will you store?** Export the audit data to a location where you can store the history. Plan to accumulate at least two years of history. That way, you can analyze trends and changes beyond the default 28-day retention period. You might choose to store the data indefinitely, or you might decide on a shorter period, depending on the data retention policies for your organization.
