...
- Introduce an intermediary data layer, such as [Dataflow Gen2](/fabric/data-factory/dataflows-gen2-overview) or [Power BI dataflows (legacy)](../transform-model/dataflows/dataflows-introduction-self-service.md) (when a data warehouse isn't a viable option). Power BI content creators can use the dataflows as their data source instead of connecting directly to data sources. An intermediary data layer can reduce the load on a source system. It also has the added benefit of centralizing data preparation logic. For more information, see the [self-service data preparation (legacy)](powerbi-implementation-planning-usage-scenario-self-service-data-preparation.md) usage scenario.
