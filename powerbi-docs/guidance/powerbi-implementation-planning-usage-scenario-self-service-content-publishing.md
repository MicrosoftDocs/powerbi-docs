...
| ![Item 13.](../media/legend-number/legend-number-13-fabric.svg) | Some, or all, Power BI items are deployed to a workspace that's dedicated to production. For a production workspace, **Fabric capacity** or **Premium capacity** workspace type is often more appropriate when there's a large number of read-only consumers. |
