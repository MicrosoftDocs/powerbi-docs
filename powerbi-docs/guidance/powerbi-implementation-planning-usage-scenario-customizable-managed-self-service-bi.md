...
As described in the [Power BI adoption roadmap](powerbi-adoption-roadmap-content-ownership-and-management.md#managed-self-service-bi), *managed self-service BI* is characterized by a blended approach that emphasizes *discipline at the core* and *flexibility at the edge*. The data architecture is maintained by a single team who are usually centralized BI experts, while reporting responsibility belongs to creators within departments or business units.

When the core data architecture doesn't include all data required, dataset creators will need to extend, personalize, or customize an existing shared dataset. The intention is to create a new specialized dataset that meets business requirements not met by existing centrally-delivered datasets.
...
The following diagram depicts a high-level overview of the most common user actions and Power BI components to support customizable managed self-service BI. The primary focus is on providing content creators in the business units with the ability to create a specialized data model by extending an existing shared dataset. The goal is to achieve reusability whenever possible and to allow flexibility meet additional analytical requirements.

:::image type="content" source="media/powerbi-implementation-planning-usage-scenario-customizable-managed-self-service-bi/usage-scenario-customizable-managed-self-service-bi.png" alt-text="Image shows a diagram of customizable managed self-service B I, which is about creating new specialized datasets by extending and personalizing existing datasets. Items in the diagram are described in the table below." lightbox="media/powerbi-implementation-planning-usage-scenario-customizable-managed-self-service-bi/usage-scenario-customizable-managed-self-service-bi-expanded.png" border="false":::

The above diagram depicts the following user actions, tools, and features:

| **Item** | **Description** |
| --- | --- |
| ![Item 1.](media/common/icon-01-red-30x30.png) | Dataset creator A develops the model using Power BI Desktop. For a dataset that's intended for reuse, it's common (but not required) for the creator to belong to a centralized team that supports users across organizational boundaries (such as IT, enterprise BI, or the Center of Excellence). |
...
| ![Item 4.](media/common/icon-04-red-30x30.png) | When ready, dataset creator A publishes the Power BI Desktop file (.pbix) that contains only a model to the Power BI service. |
...
| ![Item 8.](media/common/icon-08-red-30x30.png) | If dataset creator B doesn't have permission, they can request [Build](../connect-data/service-datasets-build-permissions.md) permission on the dataset. This starts a workflow to request Build permission from an authorized approver. |
...
| ![Item 10.](media/common/icon-10-red-30x30.png) | Power BI Desktop connects to data from additional data sources. The goal is to augment the shared dataset so that additional analytical requirements can be met with a new specialized dataset. |
| ![Item 11.](media/common/icon-11-red-30x30.png) | Relationships are created in Power BI Desktop between the existing tables (from the shared dataset, also known as the remote model) and new tables just imported (stored in the [local model](../connect-data/desktop-directquery-datasets-azure-analysis-services.md#using-directquery-for-live-connections)). Additional calculations and modeling work is done in Power BI Desktop to complete the model design. |
| ![Item 12.](media/common/icon-12-red-30x30.png) | When ready, dataset creator B publishes the Power BI Desktop file to the Power BI service. |
...
| ![Item 14.](media/common/icon-14-red-30x30.png) | The specialized dataset remains connected to the original Power BI shared dataset. Any changes to the original shared dataset will affect [downstream specialized datasets that have a dependency on it](../connect-data/desktop-directquery-datasets-azure-analysis-services.md#chaining). |
