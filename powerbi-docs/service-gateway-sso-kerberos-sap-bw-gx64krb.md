...
`gx64krb5` \ `gsskrb5` must be in use by both the client and server to complete an SSO connection through the gateway, that is, both the client and server must be using the same SNC library.
...
1. If your BW server hasn't already been configured for SSO using gx64krb5/gsskrb5, put another copy on your SAP BW server machine in a location accessible by the SAP BW server. 
...
Complete this section if you haven't already configured your SAP BW server for SNC communication (for example, SSO) using gx64krb5/gsskrb5.
...
1. Sign in to your server in SAP Logon and set the following profile parameters by using the RZ10 transaction:
    ...
    1. Set the snc/gssapi\_lib profile parameter to \<path to gsskrb5.dll/gx64krb5.dll on the BW server machine (the library you'll use depends on OS bitness)\>. Put the library in a location the SAP BW Application Server can access.
...
1. Double-click the connection you just created to attempt an SSO connection to your SAP BW server. If this connection succeeds, continue to the next step. Otherwise, review the earlier steps in this document to make sure they've been completed correctly, or review the troubleshooting section below. Note that if you can't connect to the SAP BW server via SSO in this context you won't be able to connect to the SAP BW server by using SSO in the gateway context.
...
- ```REG ADD HKLM\SOFTWARE\Wow6432Node\SAP\gsskrb5 /v ForceIniCredOK /t REG\_DWORD /d 1 /f```

- ```REG ADD HKLM\SOFTWARE\SAP\gsskrb5 /v ForceIniCredOK /t REG\_DWORD /d 1 /f```
...
* Viewing the server logs (…work\dev\_w0 on the server machine) can be helpful in troubleshooting any errors you encounter as you complete the gx64krb5/gsskrb5 setup steps. Especially if the SAP BW server won't start after the profile parameters have been changed.
...
* You might get the following message: "(SNC error) the specified module could not be found." This error can be caused by putting the `gsskrb5.dll/gx64krb5.dll` somewhere that requires elevated privileges (administrator rights) to access.
...
1. Turn on SAP BW tracing, and review the generated log files. There are several different types of SAP BW tracing available. For more information, see the SAP documentation.
