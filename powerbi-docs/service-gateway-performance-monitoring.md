...
1. **QueryExecutionReportOn**: update to _True_ to enable additional logging for queries executed using the gateway. Enabling this option creates both the query execution and the query execution aggregation report files.
...
1. **SystemCounterReportOn**: update to _True_ to enable additional logging for memory and CPU system counters. Enabling this option creates the System Counter Aggregation Report file.
...
1. There are other values in the config file that might be of interest. Update as appropriate.
...
1. After you've made the changes to the config file, restart the gateway for these config values to take effect. You'll now start seeing the report files getting generated in the location you specified for **ReportFilePath**.
...
Now, you can visualize the data that in the log files.
...
1. In the dialog box that open, check that the folder path matches the value in **ReportFilePath**.
