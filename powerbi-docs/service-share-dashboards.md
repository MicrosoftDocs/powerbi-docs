...
Watch Amanda share their dashboard with colleagues inside and outside their company. Then follow the step-by-step instructions below the video to try it out yourself.
...
Only your direct recipients can see the shared dashboard or report. For example, if you sent the email to Vicki@contoso.com, only Vicki can see the dashboard. No one else can see the dashboard, even if they have the link. Vicki must use the same email address to access it; if they sign up with any other email address, they won't have access to the dashboard.
