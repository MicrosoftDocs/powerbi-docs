...
When specifying EffectiveUserName you also specify one or more workspace roles in the Roles connection string property. If the impersonated username is a workspace admin, that username doesn't need to be a member of the specified roles. If the impersonated username is not a workspace admin, the username must belong to one or more specified roles, otherwise an error is returned.
