...
| **[Max Offline Semantic model Size (GB)](#max-offline-dataset-size)** | The maximum size of the offline semantic model in memory. This is the compressed size on disk. The default value is 0, which is the highest limit defined by SKU. The allowable range is between 0 and the capacity size limit. |
