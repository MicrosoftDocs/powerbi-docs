...
This article describes data types supported in Power BI Designer and Data Analysis Expressions (DAX). ﻿

**Note:** Just like Power BI Designer is Preview edition, so is this article. Information provided here can change often as new features in the designer become available.

When you bring data into Power BI Designer, it will attempt to convert the data type of the source column into a data type that better supports more efficient storage, calculations, and data visualization. For example, if a column of values you import from Excel has no fractional values, the designer will convert the entire column of data to a Whole Number data type, which is better suited for storing integers.
...
In Power BI Designer, you can determine and specify a column’s data type either in Query View or Report View:

![](media/powerbi-designer-data-types/PBIDataTypes_menus.png) 

The Data Type drop down in Query View has a couple more data types than Report View.  In particular, **Date/Time/Timezone** and **Duration**.When a column with these data types are loaded into the model and viewed in Report view, a column with a Date/Time/Timezone data type will be converted into a Date/Time, and a column with a Duration data type is converted into a Decimal Number.

### Number Types

Power BI Designer supports three number types:
...
Power BI Designer supports five Date/Time data types in Query View and three in the Report View and model.   Both Date/Time/Timezone and Duration are converted during load into the model.
...
**Blank** - Is a data type in DAX that represents and replaces SQL nulls. You can create a blank by using the [BLANK](https://support.office.com/en-us/article/BLANK-Function-DAX-653C1E9C-5706-433B-B360-9E4F69AAB4DA) function, and test for blanks by using the [ISBLANK](https://support.office.com/en-us/article/isblank-function-dax-ab90d35c-f9e2-4ac6-8632-5dc457bff51f) logical function.
...
|REAL|REAL|REAL|REAL|Date/time|Date/time|
...
|Operator (/)<br />(Row/Column)|INTEGER|CURRENCY|REAL|Date/time|
|---|---|---|---|---|
