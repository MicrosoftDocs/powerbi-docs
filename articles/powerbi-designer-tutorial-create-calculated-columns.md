...
Calculated columns are similar to measures in that both are based on a DAX formula, but they differ in how they are used. Measures are most often used in the Values area of a visualization, to calculate results based on other fields you have on a row in a table, or in an Axis or Group area of a visualization. Calculated columns on the other hand are used when you want the column’s results on that row in the table, or in the Axis or Group area.

This tutorial will guide you through understanding and creating some of your own calculated columns in Power BI Designer. It’s intended for Power BI users already familiar with using the designer to create more advanced models. You should already be familiar with using Query to import data, working with multiple related tables, and adding fields to the Report Canvas. If you’re new to the designer, be sure to check out [Getting Started with Power BI Designer](https://support.powerbi.com/knowledgebase/articles/471664-getting-started-with-power-bi-designer).
...
 ![](media/powerbi-designer-tutorial-create-calculated-columns/CalcCol_Fileds_NoNewCol.png)
...
![](media/powerbi-designer-tutorial-create-calculated-columns/CalcCol_Fileds_NewColumn.png)


The formula bar appears along the top of the Report Canvas. This is where we can rename our column and enter a DAX formula.

![](media/powerbi-designer-tutorial-create-calculated-columns/CalcCol_Fileds_NewColumnFormula.png)
...
Now we can begin entering our formula. We want the values in our new column to start with the ProductCategory name from the ProductCategory table. Because this column is in a different, but related table, we’re going to use the [RELATED](https://support.office.com/en-in/article/RELATED-Function-DAX-5d0eee69-8acd-4c3e-a0af-ff23ba01a7bf) function to help us get it.
...
 ![](media/powerbi-designer-tutorial-create-calculated-columns/CalcCol_Fileds_PFC_1.png)
...
 ![](media/powerbi-designer-tutorial-create-calculated-columns/CalcCol_Fileds_PFC_2.png)
...
> **Tip:** Syntax errors are most often caused by a missing or misplaced closing parenthesis. But often the designer will add it if you forget.

![](media/powerbi-designer-tutorial-create-calculated-columns/CalcCol_Fileds_PFC_3.png)
...
![](media/powerbi-designer-tutorial-create-calculated-columns/CalcCol_Fileds_PFC_4.png)
...
![](media/powerbi-designer-tutorial-create-calculated-columns/CalcCol_Fileds_PFC_6.png)
...
![](media/powerbi-designer-tutorial-create-calculated-columns/CalcCol_Fileds_Report_1.png)
...
The Contoso Sales for Power BI Designer model contains sales data for both active and inactive stores. We want to make it really clear that data shown for inactive stores is identified as such. In-effect, we want a field named Active StoreName. To do this, we’ll create another column. In this case, when a store is inactive, we want our new Active StoreName column (as a field) to show the store’s name as “Inactive”, but show the store’s real name when it’s an active store.
...
 ![](media/powerbi-designer-tutorial-create-calculated-columns/CalcCol_ActiveStore_2.png)
...
![](media/powerbi-designer-tutorial-create-calculated-columns/CalcCol_ActiveStore_3.png)
...
![](media/powerbi-designer-tutorial-create-calculated-columns/CalcCol_ActiveStore_Step5.png)

**6.**  We need to add a value for when the result is false, in this case we want to the value to be **“Inactive”**.

![](media/powerbi-designer-tutorial-create-calculated-columns/CalcCol_ActiveStore_Step6.png)
