...
There are many published articles describing how to define row level dynamic security with **SQL Server Analysis Services (SSAS) tabular** model. [For our sample we follow this article.](https://msdn.microsoft.com/en-IN/library/hh479759.aspx) The following steps walk you through the first task in this tutorial.
...
1.  Once your relational data warehouse is in place, it’s time to define your tabular model. The model can be created using **SQL Server Data Tools (SSDT)**. To get more information about how to define a tabular model, please [refer this article](https://msdn.microsoft.com/en-us/library/hh231689.aspx).
...
10.  Now we need to process and deploy the model. You can refer [this article](https://msdn.microsoft.com/en-us/library/hh231693.aspx) for assistance in deploying the model.
...
2.  To allow the **Power BI service** access your on-premises analysis service, you need to have the **[Power BI enterprise gateway](https://powerbi.microsoft.com/en-us/documentation/powerbi-gateway-enterprise/)** installed and configured in your environment.

3.  Once the **Power BI Enterprise Gateway** is correctly configured, you need to create a data source connection for your **Analysis Services** tabular instance. This article will help you with [adding data source with in the Power BI portal](https://powerbi.microsoft.com/en-us/documentation/powerbi-gateway-enterprise-manage-ssas/).
...
5.  You can also see below the DAX query which that is getting executed to populate the data for the report.
	 
	EVALUATE
	  ROW(
